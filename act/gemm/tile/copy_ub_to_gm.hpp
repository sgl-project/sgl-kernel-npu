...
struct TileCopyTla<
    ...
    Tensor<AscendC::GlobalTensor<ElementDst>, LayoutDst_,
  ...
  using TensorDst = Tensor<AscendC::GlobalTensor<ElementDst>, LayoutDst,
                           AscendC::TPosition::GM>;
  ...
  void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor) {
    AscendC::DataCopyExtParams dataCopyParams(
        ...
        (get<0>(srcTensor.stride()) - get<1>(srcTensor.shape())) /
            ELE_NUM_PER_C0,
        (get<0>(dstTensor.stride()) - get<1>(dstTensor.shape())) *
            sizeof(ElementSrc),
        0);
...
struct TileCopyTlaExt<Arch::AtlasA2,
  ...
  void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor) {
    AscendC::DataCopyExtParams dataCopyParams(
        ...
        (get<0>(srcTensor.stride()) - get<1>(srcTensor.shape())) /
            ELE_NUM_PER_C0,
        (get<1, 1>(dstTensor.stride()) - get<1, 0>(dstTensor.shape())) *
            sizeof(ElementSrc),
        0);
    AscendC::DataCopyPad(dstTensor.data(), srcTensor.data(), dataCopyParams);
  };
