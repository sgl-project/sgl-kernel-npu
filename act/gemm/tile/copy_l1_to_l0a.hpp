...
struct CopyL1ToL0A<ArchTag, Act::Gemm::GemmType<Element, layout::zN>,
                   Act::Gemm::GemmType<Element, layout::zZ>> {
...
template <class ArchTag, class Element>
struct CopyL1ToL0A<ArchTag, Act::Gemm::GemmType<Element, layout::nN>,
  ...
  void operator()(AscendC::LocalTensor<Element> dstTensor,
    ...
    loadDataParams.addrMode = 0;
    for (uint32_t i = 0; i < layoutDst.shape(1); i++) {
      AscendC::LoadData(dstTensor[i * layoutSrc.stride(3)],
...
struct CopyL1ToL0A<ArchTag, Act::Gemm::GemmType<float, layout::nN>,
  ...
  void operator()(AscendC::LocalTensor<Element> dstTensor,
    ...
    loadDataParams.dstFracGap =
        static_cast<uint16_t>(layoutSrc.shape(1) / 2) - 1;
    for (uint32_t i = 0; i < layoutDst.shape(1); i++) {
      AscendC::LoadDataWithTranspose(dstTensor[i * layoutSrc.stride(3)],
...
template <class ArchTag>
struct CopyL1ToL0A<ArchTag, Act::Gemm::GemmType<int8_t, layout::nZ>,
  ...
  void operator()(AscendC::LocalTensor<Element> dstTensor,
    ...
    for (uint32_t i = 0; i < KLoops; i++) {
      AscendC::LoadDataWithTranspose(
          dstTensor[i * MRound * KL0Alignment],
...
struct CopyL1ToL0A<
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    loadDataParams.repeatTimes = static_cast<uint16_t>(
        CeilDiv<C0_NUM_PER_FRACTAL>(layoutDst.orgShape(1)));
    loadDataParams.srcStride = layoutSrc.stride(3) / ELE_NUM_PER_FRACTAL;
    loadDataParams.sid = 0;
    loadDataParams.dstGap = layoutDst.stride(3) / ELE_NUM_PER_FRACTAL - 1;
    loadDataParams.ifTranspose = true;
...
struct CopyL1ToL0A<ArchTag,
                   Gemm::GemmType<int8_t, layout::nZ, AscendC::TPosition::A1>> {
  using Element = int8_t;
  using LayoutDst = layout::zZ;
  using LayoutSrc = layout::nZ;
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    loadDataParams.dstGap = 0;
    loadDataParams.dstFracGap =
        CeilDiv<ELE_NUM_PER_C0>(layoutDst.orgShape(1)) - 1;
...
struct TileCopyTla<
  ...
  using LayoutSrc = LayoutSrc_;
  using TensorDst =
      Tensor<AscendC::LocalTensor<Element>, LayoutDst, AscendC::TPosition::A2>;
  using TensorSrc =
  ...
  void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor) {
    ...
    for (uint32_t i = 0; i < srcOuterShapeRow; i++) {
      AscendC::LoadDataWithTranspose(
          dstTensor.data()[i * dstOuterStrideRow * 2],
