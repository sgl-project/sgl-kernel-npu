...
template <uint8_t MODE, int32_t CORE_TYPE> struct BarrierFlag {
  static_assert(MODE != MODE, "Unsupporteded cross core barrier flag, can not "
...
template <uint8_t MODE, pipe_t PIPE> ACT_DEVICE void CrossCoreBarrier() {
...
ACT_DEVICE void
CrossCoreSetFlagWithReverse(CrossCoreFlagWithReverse<REVERSE_DEPTH> &flag) {
  AscendC::CrossCoreSetFlag<MODE, PIPE>(flag.id);
...
template <uint8_t MODE, pipe_t PIPE, uint32_t REVERSE_DEPTH>
ACT_DEVICE void
CrossCoreWaitFlagWithReverse(CrossCoreFlagWithReverse<REVERSE_DEPTH> &flag) {
  AscendC::CrossCoreWaitFlag(flag.id);
  if (++flag.count >= REVERSE_DEPTH) {
