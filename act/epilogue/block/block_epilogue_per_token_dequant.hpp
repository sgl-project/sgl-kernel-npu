...
template <uint32_t UB_STAGES_, uint32_t EXEC_FLAG_, class CType_,
          class ScaleType_, class PerTokenScaleType_, class DType_,
          class TileRowBroadcastMul_, class TileBroadcastOneBlk_,
          class TileOneBlkColumnBroadcastMul_, class TileCopy_,
          class EpilogueTileSwizzle_>
class BlockEpilogue<
    EpilogueAtlasA2PerTokenDequant<UB_STAGES_, EXEC_FLAG_>, CType_, ScaleType_,
...
public:
  ...
  static_assert(
      std::is_same_v<ElementC, int32_t> &&
          (std::is_same_v<ElementD, half> ||
           std::is_same_v<ElementD, bfloat16_t>) &&
  ...
  struct Params {
    ...
    Params(__gm__ ElementScale *ptrScale_, LayoutScale const &layoutScale_,
        ...
        : ptrScale(ptrScale_), layoutScale(layoutScale_),
          ...
          layoutPerTokenScale(layoutPerTokenScale_), ptrD(ptrD_),
          layoutD(layoutD_) {}
  };
  ...
  BlockEpilogue(Arch::Resource<ArchTag> const &resource,
    ...
    for (uint32_t i = 0; i < UB_STAGES; ++i) {
      ...
      ubScaleList[i] =
          resource.ubBuf.template GetBufferByByte<ElementScale>(ubOffset);
      ubOffset += TileShape::COLUMN * sizeof(ElementScale);
      ubPerTokenScaleList[i] =
          resource.ubBuf.template GetBufferByByte<ElementPerTokenScale>(
              ubOffset);
  ...
  ACT_DEVICE
  void operator()(GemmCoord const &blockShapeMNK,
    ...
    for (uint32_t loopIdx = subblockIdx; loopIdx < tileLoops;
         loopIdx += subblockNum) {
      ...
      auto layoutUbScale =
          LayoutScale::template MakeLayoutInUb<ElementScale>(scaleTileShape);

      AscendC::WaitFlag<AscendC::HardEvent::V_MTE2>(
          eventUbScaleVMTE2List[ubListId]);
      copyGmToUbScale(ubScale, gmTileScale, layoutUbScale, layoutGmTileScale);
      AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(
          eventUbScaleMTE2VList[ubListId]);

      auto perTokenScaleTileOffset = tileOffset.template GetCoordByAxis<0>();
      auto perTokenScaleTileShape =
          actualTileShape.template GetCoordByAxis<0>();

      auto gmTilePerTokenScale =
          gmPerTokenScale[params.layoutPerTokenScale.GetOffset(
              perTokenScaleTileOffset)];
      auto layoutGmTilePerTokenScale =
          params.layoutPerTokenScale.GetTileLayout(perTokenScaleTileShape);
      ...
      copyGmToUbPerTokenScale(ubPerTokenScale, gmTilePerTokenScale,
                              layoutUbPerTokenScale, layoutGmTilePerTokenScale);
      AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(
          eventUbPerTokenScaleMTE2VList[ubListId]);

      AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(
          eventUbCMTE2VList[ubListId]);
      AscendC::Cast(ubCFp32, ubC, AscendC::RoundMode::CAST_RINT,
                    TileShape::COUNT);
      AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(eventUbCVMTE2List[ubListId]);

      AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(
          eventUbScaleMTE2VList[ubListId]);
      AscendC::Cast(ubScaleFp32, ubScale, AscendC::RoundMode::CAST_NONE,
                    TileShape::COLUMN);
      AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(
          eventUbScaleVMTE2List[ubListId]);
      ...
      AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(
          eventUbPerTokenScaleMTE2VList[ubListId]);
      AscendC::Cast(ubPerTokenScaleFp32, ubPerTokenScale,
                    AscendC::RoundMode::CAST_NONE, TileShape::ROW);
      AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(
          eventUbPerTokenScaleVMTE2List[ubListId]);
      ...
      tileOneBlkColumnBroadcastMul(ubPerTokenMul, ubMul,
                                   ubPerTokenScaleFp32Brcb);
      AscendC::PipeBarrier<PIPE_V>();
      ...
      AscendC::WaitFlag<AscendC::HardEvent::MTE3_V>(
          eventUbDMTE3VList[ubListId]);
      AscendC::Cast(ubD, ubPerTokenMul, AscendC::RoundMode::CAST_RINT,
                    TileShape::COUNT);
...
template <uint32_t UB_STAGES_, uint32_t EXEC_FLAG_, class CType_,
          class LayoutScale_, class LayoutPerTokenScale_, class DType_,
          class TileRowBroadcastMul_, class TileBroadcastOneBlk_,
          class TileOneBlkColumnBroadcastMul_, class TileCopy_,
          class EpilogueTileSwizzle_>
class BlockEpilogue<EpilogueAtlasA2PerTokenDequant<UB_STAGES_, EXEC_FLAG_>,
                    CType_, Gemm::GemmType<float, LayoutScale_>,
                    Gemm::GemmType<float, LayoutPerTokenScale_>, DType_,
                    TileRowBroadcastMul_, TileBroadcastOneBlk_,
                    TileOneBlkColumnBroadcastMul_, TileCopy_,
...
public:
  ...
  BlockEpilogue(Arch::Resource<ArchTag> &resource,
    ...
    if constexpr (EXEC_FLAG & EXEC_FLAG_DEEP_FUSE) {
      ...
      uint32_t epSendCountSize = calcInfo.isSharedExpert_
                                     ? calcInfo.epWorldSize_
                                     : calcInfo.moeSendNum_;
      AscendC::DataCopyExtParams epSendCntParams = {
          1U, static_cast<uint32_t>(epSendCountSize * sizeof(uint32_t)), 0U, 0U,
          0U};
      AscendC::DataCopyPadExtParams<int32_t> copyPadParams{false, 0U, 0U, 0U};
      AscendC::DataCopyPad(epSendCountLocal_, epSendCountGM, epSendCntParams,
                           copyPadParams);
      ...
      tokenToEpRankHashLocal_ =
          resource.ubBuf.template GetBufferByByte<int32_t>(ubOffset);
      uint32_t maxGroupSendCount = 0;
      uint32_t groupSendCount = 0;
      for (uint32_t expertIdx = 0; expertIdx < calcInfo.moeExpertPerRankNum_;
           ++expertIdx) {
        uint32_t prevGroupSendCount = groupSendCount;
        groupSendCount = epSendCountLocal_.GetValue(
            (expertIdx + 1) * calcInfo.epWorldSize_ - 1);
  ...
  ACT_DEVICE GM_ADDR GetWinAddrByRankId(const int32_t rankId,
                                        const uint8_t expertLocalId = 0U) {
    return (GM_ADDR)((calcInfo.epRankId_ == rankId)
                         ? calcInfo.epWinContext_->localWindowsIn
                         : ((HcclRankRelationResV2 *)(calcInfo.epWinContext_
                                                          ->remoteRes[rankId]
                                                          .nextDevicePtr))
                               ->windowsIn) +
           calcInfo.winDataSizeOffset_ +
           expertLocalId * calcInfo.expertPerSizeOnWin_ +
           rankId * OPT_RANK_OFFSET;
  }
#if ENABLE_EP_SEND_COUNT_HASH
  ACT_DEVICE void InitTokenToEpRankHashLocalForEpRank(uint32_t &hashOffset,
                                                      uint32_t epRank,
                                                      uint32_t copyLen) {
    constexpr uint32_t DUPLICATE_MASK_COUNT = 8;
    uint32_t hashOffsetMask =
        (((uint32_t)hashOffset) & (DUPLICATE_MASK_COUNT - 1));
    if (hashOffsetMask != 0) {
      ...
      AscendC::Duplicate<int32_t>(
          tokenToEpRankHashLocal_[hashOffset - hashOffsetMask], epRank,
          &copyMask, 1, 1, DUPLICATE_MASK_COUNT);
      hashOffset += remainMaskCount;
      copyLen -= remainMaskCount;
    }
    if (copyLen > 0) {
      AscendC::Duplicate<int32_t>(tokenToEpRankHashLocal_[hashOffset], epRank,
                                  copyLen);
      hashOffset += copyLen;
    }
  }
  ...
  ACT_DEVICE void SetCombineSendEpRank(uint32_t epRank, uint32_t &remoteEpRank,
                                       uint32_t &localEpRank) {
    if ((calcInfo.isSharedExpert_) &&
  ...
  ACT_DEVICE void DoCombineSend(AscendC::LocalTensor<ElementD> &ubD,
                                layout::RowMajor &layoutGmTileD,
                                LayoutD &layoutUbD, int64_t groupOffsetD,
                                uint32_t expertIdx, uint32_t tileOffsetD) {
    const uint32_t copyTokenLen = layoutGmTileD.shape(1) * sizeof(ElementD);
    const uint32_t copyTokenSrcStride =
        (layoutUbD.stride(0) - layoutUbD.shape(1)) /
        (BYTE_PER_C0 / sizeof(ElementD));
    const uint32_t copyTokenDstStride =
        (layoutGmTileD.stride(0) - layoutGmTileD.shape(1)) * sizeof(ElementD);
...
#endif
    uint32_t sendCount =
        expertIdx == 0 && epRankStart == 0
            ? 0
            : epSendCountLocal_.GetValue(expertOffset + epRankStart - 1);
    for (uint32_t epRank = epRankStart;
         epRank < calcInfo.epWorldSize_ && itToken < endToken; ++epRank) {
      ...
      if (prevSendCount <= itToken && itToken < sendCount) {
        uint32_t copyTokenCount =
            (sendCount < endToken ? sendCount : endToken) - itToken;
        AscendC::DataCopyExtParams dataCopyParams(copyTokenCount, copyTokenLen,
                                                  copyTokenSrcStride,
        ...
        GM_ADDR rankGM = GetWinAddrByRankId(remoteEpRank, expertIdx) +
                         localEpRank * calcInfo.moeExpertPerRankNum_ *
                             calcInfo.expertPerSizeOnWin_;
        AscendC::GlobalTensor<ElementD> rankWindow;
        rankWindow.SetGlobalBuffer((__gm__ ElementD *)rankGM);
        AscendC::DataCopyPad(
            rankWindow[(itToken - prevSendCount) * calcInfo.axisH_ +
                       tokenOffset],
  ...
  ACT_DEVICE
  void operator()(int64_t groupOffsetD, uint32_t expertIdx,
    ...
    if constexpr (EXEC_FLAG & EXEC_FLAG_DEEP_FUSE) {
      ...
      if (currentExpertIdx_ != expertIdx) {
        ...
        uint32_t sendCount =
            expertIdx == 0 ? 0 : epSendCountLocal_.GetValue(expertOffset - 1);
        for (uint32_t epRank = 0; epRank < calcInfo.epWorldSize_; ++epRank) {
          uint32_t prevSendCount = sendCount;
          sendCount = epSendCountLocal_.GetValue(expertOffset + epRank);
          InitTokenToEpRankHashLocalForEpRank(hashOffset, epRank,
    ...
    for (uint32_t loopIdx = subblockIdx; loopIdx < tileLoops;
      ...
      if constexpr (EXEC_FLAG & EXEC_FLAG_DEEP_FUSE) {
        DoCombineSend(ubD, layoutGmTileD, layoutUbD, groupOffsetD, expertIdx,
                      tileOffsetD);
      } else {
