...
template <class ArchTag, class GmType> struct CopyUb2Gm {
  static_assert(
      ...
      "Unsupporteded copy ub to gm, can not find the specialization.");
};
...
struct CopyUb2Gm<Arch::AtlasA2, Gemm::GemmType<Element, layout::VectorLayout>> {
  ...
  void operator()(AscendC::GlobalTensor<Element> const &dstTensor,
    ...
    AscendC::DataCopyExtParams dataCopyParams(
        1, layoutDst.shape(0) * sizeof(Element), 0, 0, 0);
    AscendC::DataCopyPad(dstTensor, srcTensor, dataCopyParams);
  };
};
...
template <class ArchTag, class GmType> struct CopyUb2GmAligned {
  static_assert(
      DEPENDENT_FALSE<ArchTag>,
      "Unsupporteded copy ub to gm aligned, can not find the specialization.");
};

template <typename Element>
struct CopyUb2GmAligned<Arch::AtlasA2,
