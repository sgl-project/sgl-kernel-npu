...
struct TileRowBroadcastMul {
  ...
  void operator()(AscendC::LocalTensor<ElementCompute> const &ubOut,
    ...
    constexpr uint32_t colNumPerCompute =
        BYTE_PER_VECTOR_FRACTAL / sizeof(ElementCompute);
    for (uint32_t rowOffset = 0; rowOffset < TileShape::ROW;
         rowOffset += rowNumPerCompute) {
      uint32_t residueM = TileShape::ROW - rowOffset;
      uint8_t repeatTimes = static_cast<uint8_t>(
          (residueM > rowNumPerCompute) ? rowNumPerCompute : residueM);
      for (uint32_t colOffset = 0; colOffset < TileShape::COLUMN;
           colOffset += colNumPerCompute) {
        uint32_t residueN = TileShape::COLUMN - colOffset;
        uint64_t mask =
            (residueN > colNumPerCompute) ? colNumPerCompute : residueN;
  ...
  void operator()(AscendC::LocalTensor<ElementCompute> const &ubOut,
    ...
    for (uint32_t rowOffset = 0; rowOffset < TileShape::ROW;
      ...
      for (uint32_t colOffset = 0; colOffset < TileShape::COLUMN;
        ...
        AscendC::Mul(ubOut[rowOffset * TileShape::COLUMN + colOffset],
                     ubIn0[rowOffset * TileShape::COLUMN + colOffset],
                     ubIn1[rowOffset * eleNumPerBlk], mask, repeatTimes,
                     repeatParams);
      }
    }
  }
