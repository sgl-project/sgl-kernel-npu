...
struct EpilogueIdentityTileSwizzle {
  ...
  ACT_DEVICE
  EpilogueIdentityTileSwizzle(MatrixCoord const &blockShape,
  ...
  ACT_DEVICE
  uint32_t GetLoops() const { return loopsMN.row() * loopsMN.column(); }
...
struct EpilogueHorizontalTileSwizzle {
  ...
  ACT_DEVICE
  EpilogueHorizontalTileSwizzle(MatrixCoord const &blockShape,
                                MatrixCoord const &tileShape)
      : blockShape(blockShape), tileShape(tileShape) {
    loopsMN = CeilDiv(blockShape, tileShape);
  }
