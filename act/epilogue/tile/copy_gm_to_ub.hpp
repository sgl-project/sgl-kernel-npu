...
template <class ArchTag, class GmType> struct CopyGm2Ub {
  static_assert(
      ...
      "Unsupporteded copy gm to ub, can not find the specialization.");
};
...
struct CopyGm2Ub<Arch::AtlasA2, Gemm::GemmType<Element, layout::VectorLayout>> {
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    AscendC::DataCopyExtParams dataCopyParams(
        1, layoutSrc.shape(0) * sizeof(Element), 0, 0, 0);
    AscendC::DataCopyPadExtParams<Element> padParams(false, 0, 0, 0);
    AscendC::DataCopyPad(dstTensor, srcTensor, dataCopyParams, padParams);
  };
};
...
template <class ArchTag, class GmType> struct CopyPerTokenScale2Ub {
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    dataCopyParams.blockLen =
        layoutSrc.shape(1) *
        sizeof(Element); // per token scale has only one column
...
template <class ArchTag, class GmType> struct CopyGm2UbAligned {
  static_assert(
      DEPENDENT_FALSE<ArchTag>,
      "Unsupporteded copy gm to ub aligned, can not find the specialization.");
};

template <typename Element>
struct CopyGm2UbAligned<Arch::AtlasA2,
