import torch
from sgl_kernel_npu.mem_cache import write_cache_indices_npu
...
def test_write_cache_indices():
    ...
    req_pool_indices = (torch.randint(pool_size//batch_size, (batch_size,), dtype=torch.int64, device = "npu")+torch.arange(batch_size, dtype=torch.int64, device = "npu")*(pool_size//batch_size)).reshape(-1)
    ...
    out_cache_loc = torch.arange(sum(extend_lens), dtype=torch.int64, device="npu").reshape(-1)
    ...
    write_cache_indices_npu(out_cache_loc, req_pool_indices, prefix_lens, seq_lens, extend_lens, prefix_tensors, req_to_token_tensor)
    torch.testing.assert_close(req_to_token_tensor, req_to_token_tensor_ref)
