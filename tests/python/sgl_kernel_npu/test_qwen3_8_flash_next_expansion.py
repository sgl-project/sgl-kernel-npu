...
from sgl_kernel_npu.qwen3_8_flash_next.expansion import expand_blocks, can_run_block_expansion
...
def scalar_oracle(blocks, positions, lengths, ratio, topk):
    result = []
    for row, position, length in zip(blocks.tolist(), positions.tolist(), lengths.tolist()):
        raw = []
        for block in row:
            raw.extend([block * ratio + offset if block >= 0 else -1 for offset in range(ratio)])
        raw = [v if 0 <= v < length else -1 for v in raw[:topk]]
        start = ((position + 1) // ratio) * ratio
        count = position + 1 - start
        raw += [start + j if j < count and start + j < length else -1 for j in range(ratio - 1)]
        raw = [v for v in raw if v >= 0] + [v for v in raw if v < 0]
        result.append(raw)
    return torch.tensor(result, dtype=torch.int64).to(torch.int32).reshape(blocks.shape[0], topk + ratio - 1)
...
def make_model_case(rows, device='cpu', ratio=4, topk=2048, pattern='full',
                    dtype=torch.int32, layout='contiguous', seed=0):
...
def expected(args):
    ...
    chunks = [reference(*(t[start:start+128] for t in cpu[:3]), *cpu[3:])
...
def assert_preconditions(blocks, positions, lengths, ratio):
    ...
    assert torch.all(((b + 1) * ratio)[valid] <= torch.minimum(p + 1, l)[:, None].expand_as(b)[valid])
    assert torch.all(l >= 0) and torch.all(l <= 2**31 - 1)
    assert torch.all(p >= -1) and torch.all(p < 2**31 - 1)

@pytest.mark.parametrize('rows', [0, 1, 4, 16, 64, 127, 128, 129, 512, 4095, 4096, 4097, 8192, 16384])
@pytest.mark.parametrize('pattern', ['full', 'short', 'mixed', 'padding', 'graph_padding'])
def test_model_shapes(rows, pattern):
    ...
    assert actual.shape == (rows, 2051) and actual.dtype == torch.int32 and actual.is_contiguous()
...
def test_parameterized_ratio_and_tail_remainders(k, ratio, path, monkeypatch):
    monkeypatch.setattr(module, '_NATIVE_MIN_ROWS', 0 if path == 'native' else 2**63-1)
    # Every remainder; complete blocks and a full padded suffix are both present.
    args = make_model_case(ratio, device='npu', ratio=ratio, topk=k*ratio, pattern='short')
    assert_preconditions(*args[:3], ratio)
    torch.testing.assert_close(expand_blocks(*args).cpu(), expected(args), atol=0, rtol=0)


@pytest.mark.parametrize('dtypes', list(itertools.product([torch.int32, torch.int64], repeat=3)))
@pytest.mark.parametrize('layout', ['contiguous', 'strided', 'broadcast'])
@pytest.mark.parametrize('path', ['triton', 'native'])
def test_dtype_and_layout(dtypes, layout, path, monkeypatch):
    monkeypatch.setattr(module, '_NATIVE_MIN_ROWS', 0 if path == 'native' else 2**63-1)
    args = list(make_model_case(7, device='npu', pattern='mixed', seed=3, layout=layout))
    # Preserve strides after conversion by slicing duplicated storage again.
    args[:3] = [torch.stack((t.to(d), t.to(d)), -1)[..., 0] if layout == 'strided'
                else t.to(d) for t, d in zip(args[:3], dtypes)]
    torch.testing.assert_close(expand_blocks(*args).cpu(), expected(args), atol=0, rtol=0)
...
def test_order_duplicates_and_int32_end(path, ratio, monkeypatch):
    monkeypatch.setattr(module, '_NATIVE_MIN_ROWS', 0 if path == 'native' else 2**63-1)
    args = list(make_model_case(3, device='npu', dtype=torch.int64, pattern='padding', ratio=ratio, topk=512*ratio))
...
@pytest.mark.parametrize('kind', ['budget', 'ratio', 'k', 'bool', 'float_parameter', 'rank', 'vector_shape', 'dtype', 'cpu', 'width_overflow'])
def test_reject_metadata(kind):
    args = list(make_model_case(2, device='npu'))
    if kind == 'budget': args[4] += 1
    elif kind == 'ratio': args[3] = 1
    elif kind == 'k': args[0] = args[0][:, :256]; args[4] = 1024
    elif kind == 'bool': args[3] = True
    elif kind == 'float_parameter': args[4] = 2048.0
    elif kind == 'rank': args[0] = args[0].flatten()
    elif kind == 'vector_shape': args[1] = args[1].reshape(1, 2)
    elif kind == 'dtype': args[0] = args[0].float()
    elif kind == 'cpu': args[2] = args[2].cpu()
    elif kind == 'width_overflow': args[3] = 2**23; args[4] = 512 * args[3]
...
def test_bounded_grid_loop(rows, monkeypatch):
    ...
    args = make_model_case(rows, device='npu', ratio=2, topk=1024, pattern='mixed', layout='broadcast', seed=7)
    actual = expand_blocks(*args)
    ...
    for start in range(0, rows, 1024):
        ...
        torch.testing.assert_close(chunk, one.expand(chunk.shape[0], -1), atol=0, rtol=0)
...
def test_int64_row_and_column_addresses(path, monkeypatch):
    ...
    for column_stride in (False, True):
        ...
        blocks.copy_(base[0])
        args = (blocks, *base[1:])
        torch.testing.assert_close(expand_blocks(*args).cpu(), expected(base), atol=0, rtol=0)
        del blocks, storage, args


@pytest.mark.parametrize('rows,k,ratio', [(1,512,4),(127,512,4),(128,512,4),(129,512,3),
                                         (4097,512,4),(8192,2048,4),(17,2048,8)])
...
def test_graph_input_updates(rows, k, ratio, path, monkeypatch):
    ...
    for _ in range(3): expand_blocks(*args)
    graph = torch.npu.NPUGraph()
    with torch.npu.graph(graph): output = expand_blocks(*args)
    for pattern in ['mixed', 'short', 'padding', 'graph_padding', 'full']:
        replacement = make_model_case(rows, device='npu', topk=k*ratio, ratio=ratio, pattern=pattern, seed=1)
        for old, new in zip(args[:3], replacement[:3]): old.copy_(new)
...
def test_native_chunk_graph(monkeypatch):
    ...
    for pattern in ['mixed','padding','graph_padding','short']:
        ...
        for a,b in zip(args[:3],replacement[:3]): a.copy_(b)
        graph.replay();torch.npu.synchronize()
        torch.testing.assert_close(output.cpu(),expected(replacement),atol=0,rtol=0)
