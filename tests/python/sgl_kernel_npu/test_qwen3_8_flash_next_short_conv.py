...
def check_output(label, out, x, w, single):
    ...
    assert tuple(out.shape) == (
        (x.shape[0], 10240) if single else (x.shape[0], width, 10240)
    )
...
def test_model_contract(dtype, width, single, batch):
    ...
    if batch:
        block = (
            (512 if batch == 1 else 1024)
            if width == 1
            else (256 if batch == 1 else 512)
        )
...
def test_contiguous_offset_and_padding(dtype, width):
    ...
    backing_x.fill_(7)
    backing_w.fill_(-3)
    sx = backing_x[3:].view_as(x)
    sx.copy_(x)
    x = sx
    sw = backing_w[5:].view_as(w)
    sw.copy_(w)
    w = sw
...
def test_graph_updates(dtype, width, single, batch, grid_mode, monkeypatch):
    ...
    block = (
        (512 if batch == 1 else 1024) if width == 1 else (256 if batch == 1 else 512)
    )
    if grid_mode != "native":
    ...
    for label, changed in [("initial", None), ("input", x), ("weight", w)]:
        ...
        torch.npu.synchronize()
        check_output(
            f"graph/B{batch}/W{width}/single{single}/{grid_mode}/{label}",
            out,
            x,
            w,
            single,
...
REJECTIONS = [
    ...
    *[
        (f"dtype_{v}", dict(dtype=v))
        for v in (torch.float32, torch.float64, torch.int32)
    ],
...
def test_reject_metadata(label, settings):
    s = settings
    x = Metadata(
        s.get("xshape", (1, 10240, 13)),
        s.get("dtype", torch.bfloat16),
        s.get("device"),
        s.get("xcontig", True),
    )
    w = Metadata(
        s.get("wshape", (10240, 1, 4)),
        s.get("wdtype", x.dtype),
        x.device,
        s.get("wcontig", True),
    )
...
def test_int32_boundary(width, delta, monkeypatch):
    ...
    x = Metadata((max_batch + delta, 10240, width + 9))
    w = Metadata((10240, 1, 4))
    if delta:
...
def test_reject_real_strided_tensor(which):
    ...
    backing = torch.empty(
        (*v.shape[:-1], v.shape[-1] * 2), device=v.device, dtype=v.dtype
    )
    backing[..., ::2] = v
    if which == "input":
        x = backing[..., ::2]
