...
def direct_testing(input_a, input_b, input_c, team_id=0, group_list=(), use_nz=False):
    ...
    for _ in range(1):
        ...
        if use_nz:
            ...
            torch.ops.npu.catcoc_allgather_matmul(a, b_nz, input_c, g_shmem_addr, team_id, format_mode="NZ")
        else:
...
def run_global_test(test_mnk=(1024, 1024, 1024), test_nz=True):
    ...
    print('+++++++++++++++++++++++Testing FP16+++++++++++++++++') if rank == 0 else None
    ...
    if test_nz:
        print('+++++++++++++++++++++++Testing FP16(WeightNZ)+++++++++++++++++') if rank == 0 else None
        c = torch.empty([test_m * world_size, test_n]).to(dtype=torch.float16).to(f"npu:{rank}").contiguous()  # fresh tensor
        direct_testing(a, b, c, use_nz=True)

    print('+++++++++++++++++++++++Testing BF16+++++++++++++++++') if rank == 0 else None
    a = torch.rand([test_m, test_k]).to(dtype=torch.bfloat16).to(f"npu:{rank}").contiguous()
    b = torch.rand([test_k, test_n]).to(dtype=torch.bfloat16).to(f"npu:{rank}").contiguous()
    c = torch.empty([test_m * world_size, test_n]).to(dtype=torch.bfloat16).to(f"npu:{rank}").contiguous()  # tmp tensor
    bias = None
    scale = None
    pertoken_scale = None
    ...
    if test_nz:
        print('+++++++++++++++++++++++Testing FP16(WeightNZ)+++++++++++++++++') if rank == 0 else None
        c = torch.empty([test_m * world_size, test_n]).to(dtype=torch.bfloat16).to(f"npu:{rank}").contiguous()  # fresh tensor
        direct_testing(a, b, c, use_nz=True)
