...
# npu-smi reports Ascend 950 from the device-level query, while A2/A3 need a chip id.
FAKE_NPU_SMI = {
    "950": 'echo "  Chip Name  : Ascend950PR"',
...
def test_explicit_soc_version_skips_detection(requested_soc, wheel_target, cmake_soc):
    ...
    done = resolve_soc("kernels", chip="950", requested_soc=requested_soc)
...
def test_source_tree_ships_no_staged_gemma_provider():
    ...
    whichever target it happens to match, so an Ascend 950 wheel would silently carry
