...
def generate_combine_inputs(
    args: argparse.Namespace, rank: int, num_ranks: int, is_low_latency: bool = False
):
    ...
    aligned_num_tokens = num_tokens
    if is_low_latency:
        local_tokens_tensor = torch.tensor(
            [num_tokens], dtype=torch.int32, device="npu"
        )
    ...
    if is_low_latency:
        rank_offset = 128
        assert num_ranks - rank_offset < 257, "Too many ranks"
        x = torch.ones((num_tokens, hidden), dtype=torch.bfloat16, device="npu") * (
            rank - rank_offset
        )
        x[:, -128:] = (
            torch.arange(num_tokens, device="npu").to(torch.bfloat16).view(-1, 1)
        )
...
def test_only_normal_combine(
    ...
    inputs = generate_combine_inputs(args, rank, num_ranks, is_low_latency=False)
...
def test_only_low_latency_combine(
    ...
    inputs = generate_combine_inputs(args, rank, num_ranks, is_low_latency=True)
    ...
    simulated_gemm_x = per_token_cast_back(*packed_recv_x) if isinstance(packed_recv_x, tuple) else packed_recv_x
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    if args.test_type == "normal":
        ...
        test_only_low_latency_combine(args, rank, num_ranks, buffer, group)
