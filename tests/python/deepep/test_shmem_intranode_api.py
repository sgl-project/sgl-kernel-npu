...
def test_main(
    ...
    for current_x in filter(lambda elem: elem is not None, (x,)):
        ...
        dispatch_t = bench(lambda: buffer.dispatch(**tune_args))[0]
        # if local_rank == 0:
        print(
    ...
    combine_t = bench(lambda: buffer.combine(**tune_args))[0]
    # if local_rank == 0:
    print(
    ...
    calculate_avg_stats(
        ...
        num_dispatch_comm_bytes=dispatch_bf16_recv_bytes,
        combine_t=combine_t,
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    rank, num_ranks, group, meta_addr = init_dist(local_rank, num_local_ranks)
