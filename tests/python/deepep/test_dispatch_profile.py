"""Profile dispatch correctness test for cam_moe_dispatch_normal_a5."""
...
def test_dispatch_profile(
    ...
    def dispatch_fn(profile_enable: bool):
    ...
    (recv_x, _, _, _, _, _) = dispatch_fn(profile_enable=False)
    if rank == 0:
        print(
            f"[rank{rank}] dispatch without profiling OK. recv_x shape={recv_x.shape}",
            flush=True,
        )
    ...
    if args.kernel_trace_dir is not None:
        ...
        try:
            for _ in range(args.num_warmups):
                dispatch_fn(profile_enable=True)
                torch.npu.synchronize()
    ...
    dist.barrier()
    if rank == 0:
        print("[all] dispatch profiling test PASSED", flush=True)
