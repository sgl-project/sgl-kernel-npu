...
for (( idx =0; idx < ${GNPU_NUM}; idx = idx + 1 )); do
    msprof --application="${TEST_ROOT}/out/bin/ascendc_reduce_scatter $RANK_SIZE $idx $IPPORT $GNPU_NUM $FIRST_RANK $FIRST_NPU $TEST_TYPE $ZERO_BUFF" --output=${CURRENT_DIR}/output/ &
    pid=$!
    pids+=("$pid")
