...
// AscendC headers MUST precede <pto/pto-inst.hpp>: pto-isa (CANN 9.0.0 line) pulls in
// `using namespace std` via pto/common/fifo.hpp, which makes AscendC's unqualified
// `conditional` in common_types.h ambiguous against std::conditional. Parsing AscendC
// first keeps `conditional` unambiguous.
//
// kernel_operator.h is included UNCONDITIONALLY (not just the CUBE pass) so that
// KERNEL_TASK_TYPE_DEFAULT / KERNEL_TYPE_MIX_AIC_1_2 are defined in the ascendc op-codegen
// *preprocess* pass too (it compiles this source with neither __DAV_C220_* macro defined).
// The MIX task-type marker must be visible there or the mix binary registers with ret
// 107000 and the cube side no-ops. The `using namespace AscendC/matmul` directives stay
// CUBE-only so they don't leak into the pto vec pass.
...
#include "acl/acl.h"
// (runtime/rt_ffts.h dropped: rtGetC2cCtrlAddr lived in the removed ctypes launcher;
//  the ascendc runtime sets the FFTS base addr before launch.)
...
#endif
#ifndef W4A4_MEGA_KERNEL_NAME
#define W4A4_MEGA_KERNEL_NAME launch_mega_moe_w4a4
#endif
...
{
    return (0x1 + ((mode & 0x3) << HX_SYNC_MODE_SHIFT_VALUE) + ((flagId & 0xf) << HX_SYNC_FLAG_SHIFT_VALUE));
}
...
static AICORE void stage1_int4_routed(__gm__ half *x_gm, __gm__ int32_t *sort_idx_gm, __gm__ int8_t *q_gm,
                                      __gm__ float *s_gm, uint32_t M_total, uint32_t top_k, uint32_t t_lo = 0,
                                      uint32_t t_hi = 0)
{
    set_mask_norm();
    set_vector_mask(-1, -1);
    ...
    for (uint32_t base = my_lo; base < my_hi; base += S1_R) {
        ...
        TF16 tmpT(rows);
        TASSIGN(tmpT, TMP_BASE);
        TMaxCm rmaxCm(rows);
        TASSIGN(rmaxCm, RMAX_CM);
        TMaxCm rminCm(rows);
        TASSIGN(rminCm, RMIN_CM);
        TROWMAX(rmaxCm, xT, tmpT);
        pipe_barrier(PIPE_V);
        TROWMIN(rminCm, xT, tmpT);
        pipe_barrier(PIPE_V);
        TMaxRm rmaxRm(rows);
        TASSIGN(rmaxRm, RMAX_CM);
        TMaxRm rminRm(rows);
        TASSIGN(rminRm, RMIN_CM);
        TRESHAPE(rmaxRm, rmaxCm);
        TRESHAPE(rminRm, rminCm);
        pipe_barrier(PIPE_V);
        TMULS(rminRm, rminRm, (half)-1.0f);
        pipe_barrier(PIPE_V);
        TMAX(rmaxRm, rmaxRm, rminRm);
        pipe_barrier(PIPE_V);
        ...
        TScRm scRm(rows);
        TASSIGN(scRm, SC_CM);
        TCVT(scRm, rmaxRm, RoundMode::CAST_NONE);
        pipe_barrier(PIPE_V);
        TMULS(scRm, scRm, (float)(kInvSqrtN / kScaleDivisor));
        pipe_barrier(PIPE_V);
        ...
        TMULS(rmaxCm, rmaxCm, (half)(1.0f / kScaleDivisor));
        pipe_barrier(PIPE_V);
        TROWEXPANDDIV(xT, xT, rmaxCm);
        pipe_barrier(PIPE_V);
        ...
        GmI8P_2D qg(q_gm + (int64_t)base * (H_DIM / 2), qShape, Stride<1, 1, 1, DYNAMIC, 1>(1, 1, 1, H_DIM / 2, 1));
        TSTORE(qg, qT);
        ...
        wait_flag(PIPE_V, PIPE_S, EVENT_ID0);
        {
            ...
            for (int32_t r = 0; r < rows; ++r)
                s_gm[(int64_t)(base + r) * 32] = sc_ub[r];
        }
...
static AICORE void stage3_int4_swiglu_quant_grouped_v2(__gm__ half *gu_gm, __gm__ float *xs_gm, __gm__ float *ws_gm,
                                                       __gm__ int64_t *gl_gm, __gm__ int8_t *iq_gm, __gm__ float *is_gm,
                                                       uint32_t T, uint32_t E, uint32_t e_lo = 0, uint32_t e_hi = 0)
{
    set_mask_norm();
    set_vector_mask(-1, -1);
    constexpr float kScaleDivisor = 7.0f;
    ...
    using GmI32_2D = GlobalTensor<int32_t, TileShape2D<int32_t, DYNAMIC, DYNAMIC, Layout::ND>,
                                  Stride<1, 1, 1, DYNAMIC, 1>, Layout::ND>;
    using GmF16_2D = GlobalTensor<half, TileShape2D<half, DYNAMIC, DYNAMIC, Layout::ND>, Stride<1, 1, 1, DYNAMIC, 1>,
                                  Layout::ND>;  // S3FP16: fp16 gate_up GM
    using GmF32_1D = GlobalTensor<float, TileShape2D<float, 1, DYNAMIC, Layout::ND>, Stride<1, 1, 1, 1, 1>, Layout::ND>;
    using GmI8P_2D = GlobalTensor<int8_t, TileShape2D<int8_t, DYNAMIC, DYNAMIC, Layout::ND>,
                                  Stride<1, 1, 1, DYNAMIC, 1>, Layout::ND>;
...
#define S3_LOAD(B, MT, ROWS)                                                                               \
    do {                                                                                                   \
        ...
        TF16 _g(_rows);                                                                                    \
        TASSIGN(_g, GATE_I32_##B); /* fp16 tile in oversized int32 slot */                                 \
        TF16 _u(_rows);                                                                                    \
        TASSIGN(_u, UP_I32_##B);                                                                           \
        TLOAD(_g, _gGm);                                                                                   \
        TLOAD(_u, _uGm);                                                                                   \
...
#define S3_QSCALE(D, RM)                             \
    do {                                             \
        TMULS(RM, RM, (half)(1.0f / kScaleDivisor)); \
        pipe_barrier(PIPE_V);                        \
        TROWEXPANDDIV(D, D, RM);                     \
        pipe_barrier(PIPE_V);                        \
    } while (0)
...
#define S3_COMPUTE(B, MT, ROWS, NEEDFREE)                                                                     \
    do {                                                                                                      \
        ...
        TXs xs_dyn(rows);                                                                                     \
        TASSIGN(xs_dyn, XS_0 + (B) * (S3_ROWS * 4));                                                          \
        {                                                                                                     \
            __ubuf__ float *_xs_ub = ub_ptr<float>(xs_dyn);                                                   \
            for (int32_t _r = 0; _r < rows; ++_r)                                                             \
                _xs_ub[_r] = xs_gm[(t0 + _r) * 32];                                                           \
        }                                                                                                     \
        set_flag(PIPE_S, PIPE_V, EVENT_ID7);                                                                  \
        wait_flag(PIPE_S, PIPE_V, EVENT_ID7);                                                                 \
        TF16 gI32_dyn(rows);                                                                                  \
        TASSIGN(gI32_dyn, GATE_I32_##B); /* fp16: cube already dequant'd */                                   \
        TF16 uI32_dyn(rows);                                                                                  \
        TASSIGN(uI32_dyn, UP_I32_##B);                                                                        \
        TF32 gF32_dyn(rows);                                                                                  \
        TASSIGN(gF32_dyn, GATE_F32);                                                                          \
        TF32 uF32_dyn(rows);                                                                                  \
        TASSIGN(uF32_dyn, UP_F32);                                                                            \
        TCVT(gF32_dyn, gI32_dyn, RoundMode::CAST_NONE);                                                       \
        pipe_barrier(PIPE_V);                                                                                 \
        TCVT(uF32_dyn, uI32_dyn, RoundMode::CAST_NONE);                                                       \
        pipe_barrier(PIPE_V);                                                                                 \
        set_flag(PIPE_V, PIPE_MTE2, (event_t)(EVENT_ID2 + (B)));                                              \
        TROWEXPANDMUL(gF32_dyn, gF32_dyn, xs_dyn);                                                            \
        pipe_barrier(PIPE_V);                                                                                 \
        TROWEXPANDMUL(uF32_dyn, uF32_dyn, xs_dyn);                                                            \
        pipe_barrier(PIPE_V);                                                                                 \
        /* S3FP16: per-channel w13 scale folded into the gate_up cube FIXPIPE; no TCOLEXPANDMUL */            \
        TF32 tmp_dyn(rows);                                                                                   \
        TASSIGN(tmp_dyn, TMP_F32);                                                                            \
        TMULS(tmp_dyn, gF32_dyn, -1.0f);                                                                      \
        pipe_barrier(PIPE_V);                                                                                 \
        TEXP(tmp_dyn, tmp_dyn);                                                                               \
        pipe_barrier(PIPE_V);                                                                                 \
        TADDS(tmp_dyn, tmp_dyn, 1.0f);                                                                        \
        pipe_barrier(PIPE_V);                                                                                 \
        TDIV(gF32_dyn, gF32_dyn, tmp_dyn);                                                                    \
        pipe_barrier(PIPE_V);                                                                                 \
        TMUL(gF32_dyn, gF32_dyn, uF32_dyn);                                                                   \
        pipe_barrier(PIPE_V);                                                                                 \
        TF16 swF16_dyn(rows);                                                                                 \
        TASSIGN(swF16_dyn, SWG_F16);                                                                          \
        TCVT(swF16_dyn, gF32_dyn, RoundMode::CAST_NONE);                                                      \
        pipe_barrier(PIPE_V);                                                                                 \
        TF16 tmpRedF16(rows);                                                                                 \
        TASSIGN(tmpRedF16, TMP_F32);                                                                          \
        TRMaxCm rmaxCm_dyn(rows);                                                                             \
        TASSIGN(rmaxCm_dyn, RMAX_CM);                                                                         \
        TRMaxCm rminCm_dyn(rows);                                                                             \
        TASSIGN(rminCm_dyn, RMIN_CM);                                                                         \
        TROWMAX(rmaxCm_dyn, swF16_dyn, tmpRedF16);                                                            \
        pipe_barrier(PIPE_V);                                                                                 \
        TROWMIN(rminCm_dyn, swF16_dyn, tmpRedF16);                                                            \
        pipe_barrier(PIPE_V);                                                                                 \
        TRMaxRm rmaxRm_dyn(rows);                                                                             \
        TASSIGN(rmaxRm_dyn, RMAX_CM);                                                                         \
        TRMaxRm rminRm_dyn(rows);                                                                             \
        TASSIGN(rminRm_dyn, RMIN_CM);                                                                         \
        TRESHAPE(rmaxRm_dyn, rmaxCm_dyn);                                                                     \
        TRESHAPE(rminRm_dyn, rminCm_dyn);                                                                     \
        pipe_barrier(PIPE_V);                                                                                 \
        TMULS(rminRm_dyn, rminRm_dyn, (half) - 1.0f);                                                         \
        pipe_barrier(PIPE_V);                                                                                 \
        TMAX(rmaxRm_dyn, rmaxRm_dyn, rminRm_dyn);                                                             \
        pipe_barrier(PIPE_V);                                                                                 \
        TScaleR isOut_dyn(rows);                                                                              \
        TASSIGN(isOut_dyn, IS_0 + (B) * (S3_ROWS * 4));                                                       \
        TCVT(isOut_dyn, rmaxRm_dyn, RoundMode::CAST_NONE);                                                    \
        pipe_barrier(PIPE_V);                                                                                 \
        TMULS(isOut_dyn, isOut_dyn, 1.0f / kScaleDivisor);                                                    \
        pipe_barrier(PIPE_V);                                                                                 \
        TRESHAPE(rmaxCm_dyn, rmaxRm_dyn);                                                                     \
        pipe_barrier(PIPE_V);                                                                                 \
        S3_QSCALE(swF16_dyn, rmaxCm_dyn);                                                                     \
        TI4P iq_dyn(rows);                                                                                    \
        TASSIGN(iq_dyn, IQ_##B);                                                                              \
        fast_hadamard_int4::TCVT_FP16_TO_INT4_PACKED(iq_dyn, swF16_dyn, RoundMode::CAST_RINT);                \
        pipe_barrier(PIPE_V);                                                                                 \
        set_flag(PIPE_V, PIPE_MTE3, EVENT_ID##B);                                                             \
        wait_flag(PIPE_V, PIPE_MTE3, EVENT_ID##B);                                                            \
    ...
    uint32_t _e0 = e_lo;
    while (_e0 < e_end3 && gl_gm[_e0] <= _my_lo)
        ++_e0;
    int64_t _cur = _my_lo;
...
#define S3_TILE_ROWS(TI) (((TI) * S3_STEP + S3_STEP <= M_e) ? S3_STEP : (M_e - (TI) * S3_STEP))
        ...
        for (int32_t ti = 0; ti < n_tiles; ++ti) {
            ...
            if (cb0) {
                S3_COMPUTE(0, mt, rows, (set_buf[0] != 0));
                set_buf[0] = 1;
                set_dif[0] = 1;
            } else {
                S3_COMPUTE(1, mt, rows, (set_buf[1] != 0));
                set_buf[1] = 1;
                set_dif[1] = 1;
            }
        }
...
static AICORE void stage5_scatter_combine_int4(__gm__ int32_t *d_gm_i32, __gm__ float *is_gm,
                                               __gm__ float * /*w2s_gm unused*/, __gm__ int32_t *sort_idx_gm,
                                               __gm__ half *topk_w_gm, __gm__ int64_t *gl_gm, __gm__ half *y_gm,
    ...
    // row-partition removes the straggler. Same total work + same contiguous d/y access.
    {
        ...
        if (n_rows < 4) {
            ...
            for (int64_t r = 0; r < n_rows; ++r) {
                ...
                if (cb0) {
                    S5F_COMPUTE(0, (set_buf[0] != 0));
                    set_buf[0] = 1;
                    set_dif[0] = 1;
                } else {
                    S5F_COMPUTE(1, (set_buf[1] != 0));
                    set_buf[1] = 1;
                    set_dif[1] = 1;
                }
            }
...
AICORE inline void grouped_matmul_int4_dequant_impl(MMImpl_dq &mm, const TCubeTiling *tilingPtr, __gm__ int8_t *a_gm,
                                                    ...
                                                    __gm__ int64_t *gl_gm, uint32_t E, uint32_t e_lo, uint32_t e_hi)
{
    ...
    yGm.SetGlobalBuffer(reinterpret_cast<__gm__ half *>(c_gm));
    GlobalTensor<uint64_t> sGm;
    sGm.SetGlobalBuffer(reinterpret_cast<__gm__ uint64_t *>(scale_gm));
...
using L1Mat_h = pto::Tile<pto::TileType::Mat, T, R, C, pto::BLayout::ColMajor, RV, CV, pto::SLayout::RowMajor, 512,
                          pto::PadValue::Zero>;
template <typename T, int R, int C, int RV = R, int CV = C>
using L1MatB_h = pto::Tile<pto::TileType::Mat, T, R, C, pto::BLayout::RowMajor, RV, CV, pto::SLayout::ColMajor, 512,
                           pto::PadValue::Zero>;

static AICORE void stage0_hadamard(__gm__ half *x_gm, __gm__ half *b1_gm, __gm__ half *xrot_gm, uint32_t T_orig)
{
    ...
    pto::TileLeft<half, M_T0, HN0, M_T0, HN0> a_l0;
    TASSIGN(a_l0, 0x0);
    pto::TileRight<half, HN0, HN0, HN0, HN0> b_l0;
    TASSIGN(b_l0, 0x0);
    pto::TileAcc<float, M_T0, HN0, M_T0, HN0> c_l0;
    TASSIGN(c_l0, 0x0);
    ...
    for (uint32_t pair = cid; pair < total; pair += num_cores) {
        ...
        L1Mat_h<half, M_T0, HN0, M_T0, HN0> a_l1;
        TASSIGN(a_l1, 0x0);
        pto::Shape<1, 1, 1, pto::DYNAMIC, HN0> aShape;
        aShape.shape[3] = M_e;
        pto::GlobalTensor<half, decltype(aShape), pto::Stride<1, 1, 1, H_DIM, 1>> aGm(
            x_gm + (int64_t)m_base * H_DIM + k_base, aShape);
        TLOAD(a_l1, aGm);
        L1MatB_h<half, HN0, HN0, HN0, HN0> b_l1;
        TASSIGN(b_l1, M_T0 * HN0 * sizeof(half));
        ...
        pto::TileAcc<float, M_T0, HN0, pto::DYNAMIC, HN0> c_view(M_e);
        TASSIGN(c_view, 0x0);
        pto::Shape<1, 1, 1, pto::DYNAMIC, HN0> cShape;
        cShape.shape[3] = M_e;
        pto::GlobalTensor<half, decltype(cShape), pto::Stride<1, 1, 1, H_DIM, 1>> cGm(
            xrot_gm + (int64_t)m_base * H_DIM + k_base, cShape,
...
W4A4_MEGA_KERNEL_NAME(GM_ADDR x_gm, GM_ADDR w13_gm, GM_ADDR w13_scale_gm, GM_ADDR w2_gm, GM_ADDR w2_scale_gm,
                      ...
                      GM_ADDR xrot_ws,       // [T_orig, H] fp16 rotated-x workspace (Stage B)
                      uint32_t M_total, uint32_t E, uint32_t top_k, uint32_t T_orig)
{
    // Declare the MIX AIC:AIV = 1:2 core ratio so the ascendc op framework registers a valid
    // mix-binary core descriptor (matches csrc/catlass matmul kernels). MUST be unconditional:
    // the codegen preprocess pass (no __DAV_C220_* macro) reads this marker. Guarding it to the
    // CUBE pass hid it from the scan -> mix registered with ret 107000 -> cube no-op -> zeros.
    KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_MIX_AIC_1_2);
    // FFTS base addr is set by the ascendc runtime before launch (see header note).
    ...
    {
        pipe_barrier(PIPE_ALL);
        int64_t th = gl_ov[eA - 1];
        pipe_barrier(PIPE_ALL);
        stage1_int4_routed(s1x, (__gm__ int32_t *)sort_idx_gm, (__gm__ int8_t *)xq_ws, (__gm__ float *)xs_ws, M_total,
                           top_k, 0u, (uint32_t)th);
    }
    HxSyncAllImpl<false>();  // B1
    {
        pipe_barrier(PIPE_ALL);
        int64_t tl = gl_ov[eA - 1], th = gl_ov[E - 1];
        pipe_barrier(PIPE_ALL);
        stage1_int4_routed(s1x, (__gm__ int32_t *)sort_idx_gm, (__gm__ int8_t *)xq_ws, (__gm__ float *)xs_ws, M_total,
    ...
    TCubeTiling tiling_gu, tiling_dn;
    {
        ...
        for (uint32_t i = 0; i < sizeof(TCubeTiling) / sizeof(int32_t); ++i)
            dst[i] = src[i];
    }
    {
