...
at::Tensor CopyTilingToDevice(const SparseAttnSharedkvTilingData &tilingData)
{
    const auto tilingSize = static_cast<int64_t>(sizeof(SparseAttnSharedkvTilingData));
    auto cpuTiling = at::empty({tilingSize}, at::TensorOptions().dtype(at::kByte).device(at::kCPU));
    std::memcpy(cpuTiling.data_ptr(), &tilingData, tilingSize);
    ...
    c10::string_view layout_q, c10::string_view layout_kv, bool return_softmax_lse)
{
    ...
    auto tilingTensor = CopyTilingToDevice(tiling.GetTilingData());
    ...
    EXEC_KERNEL_CMD(sparse_attn_sharedkv, tiling.GetBlockDim(), q, oriKvLaunch, cmpKvLaunch, oriSparseLaunch,
