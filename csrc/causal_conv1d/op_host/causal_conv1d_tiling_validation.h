...
inline ge::graphStatus GetAttrsInfo(gert::TilingContext *context, CausalConv1dAttrInfo &attrInfo)
{
    ...
    OP_CHECK_IF(attrInfo.activationMode != 0 && attrInfo.activationMode != 1,
                OP_LOGE(context, "activationMode only supports 0/1"), return ge::GRAPH_FAILED);
                                         ...
                                         CausalConv1dTilingData &tiling, bool &hasBias)
{
    ...
    OP_CHECK_IF(ValidateAlignedDim(context, dim) != ge::GRAPH_SUCCESS,
                OP_LOGE(context, "dim alignment validation failed"), return ge::GRAPH_FAILED);
    ...
    if (ismShapePtr != nullptr) {
        ...
        if (!ismAbsent) {
            OP_CHECK_IF(isDecodeMode, OP_LOGE(context, "initialStateMode is only supported in runMode=0 (fn/prefill)"),
                        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(xDtype != ge::DT_BF16 && xDtype != ge::DT_FLOAT16, OP_LOGE(context, "x dtype only supports bf16/fp16"),
                return ge::GRAPH_FAILED);
