...
static ge::graphStatus CausalConv1dTilingFunc(gert::TilingContext *context)
{
    ...
    if (isFn) {
        ...
        if (tiling->inputMode == 0) {
            ...
            if (fnHostPlan.tokenSeqRangePlan.enabled) {
                ...
                OP_LOGD(context, "FnTokenSeqRanges disabled: tokenBlockCnt[%ld] exceeds fixed tiling capacity[%ld].",
        ...
        OP_LOGD(context,
                ...
                tiling->hasExplicitTokenSeqRanges, baseDimChoice.gridSize, fnHostPlan.tokenBlockChoice.gridSize,
                fnHostPlan.tokenCoreMapping.blockDim);
    }

    uint32_t blockDim =
        (effectiveGridSize < static_cast<int64_t>(coreNum)) ? static_cast<uint32_t>(effectiveGridSize) : coreNum;
    if (isFn) {
        const int64_t mappedBlockDim = (effectiveGridSize < fnHostPlan.tokenCoreMapping.blockDim)
                                           ? effectiveGridSize
                                           : fnHostPlan.tokenCoreMapping.blockDim;
    ...
    if (isFn && tiling->hasInitialStateMode != 0) {
        ...
        const int64_t workspaceSize = ASCENDC_RESERVED_WORKSPACE_SIZE + syncWorkspaceSize + snapshotWorkspaceSize;
        ...
        OP_CHECK_IF(context->SetScheduleMode(1) != ge::GRAPH_SUCCESS, OP_LOGE(context, "SetScheduleMode(1) error"),
                    return ge::GRAPH_FAILED);
