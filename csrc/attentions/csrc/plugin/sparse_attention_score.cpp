...
constexpr std::string_view SPARSE_ATTENTION_SCORE_NAME = "aclnnSparseAttentionScore";
    ...
    int64_t top_k, int64_t inner_precise)
{
    ...
    CheckParams(query, key, value, select_idx, block_table, select_num_idx,
                q_dequant_scale, k_dequant_scale, v_dequant_scale,
                actual_seq_lengths, actual_seq_lengths_kv, num_key_value_heads,
                block_size, top_k, inner_precise);
    ...
    EXEC_NPU_CMD<SPARSE_ATTENTION_SCORE_NAME>(
        query, key, value, select_idx, block_table, select_num_idx,
        actual_seq_lengths, actual_seq_lengths_kv, q_dequant_scale,
        k_dequant_scale, v_dequant_scale, num_key_value_heads, scale_value,
        block_size, top_k, inner_precise, attentionOut, softmaxLse);
