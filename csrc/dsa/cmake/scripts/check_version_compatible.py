...
class VersionChecker:
    ...
    def main(cls) -> NoReturn:
        ...
        # 参数注册
        parser.add_argument(
            "--cann_path", required=True, nargs=1, type=str, help="CANN install path"
        )
        parser.add_argument(
            "--cann_package_name",
            required=True,
        ...
        # 子命令行(Check)
        p_chk = sub_parser.add_parser(
            "check_code_compatible", help="Check Version Compatible."
        )
        p_chk.add_argument(
            "--code_version_info_file",
            required=True,
            nargs=1,
            type=str,
        ...
        # 基本合法性检查, 版本号获取
        cann_version_info_file = Path(
            args.cann_path[0], args.cann_package_name[0], "version.info"
        ).absolute()
