...
const std::map<std::string, std::vector<ge::DataType>> DTYPE_SUPPORT_MAP = {
    ...
    {VALUE_NAME, {ge::DT_FLOAT16, ge::DT_BF16}},      {SPARSE_INDICES_NAME, {ge::DT_INT32, ge::DT_INT32}},
    {QUERY_ROPE_NAME, {ge::DT_FLOAT16, ge::DT_BF16}}, {KEY_ROPE_NAME, {ge::DT_FLOAT16, ge::DT_BF16}},
...
void SFAMlaTiling::FillTilingSplitKVMla()
{
    ...
    tilingData_.splitKVParams.set_logSumExpSize(
        2 * aicNum_ * 2 * sfaInfo_->n2Size * mBaseSize_ *  // 2:每个核可能有头规约和尾规约，一共两份规约信息;sum + max
                                             ...
                                             const SFALayout &layout, const std::string &name) const
{
    ...
    if (shape.GetDimNum() != shapeExpected.GetDimNum()) {
        OPS_LOG_E(opName_, "%s shape.dim is %zu, expected shape.dim is %zu, they should be equal.", name.c_str(),
                  shape.GetDimNum(), shapeExpected.GetDimNum());
        return ge::GRAPH_FAILED;
    }
...
template <typename T>
void SFATilingCheck::LogErrorAttrValueSupport(const std::vector<T> &expectNumberList, const T &actualValue,
                                              const std::string &name) const
{
                                           ...
                                           const SFALayout &actualLayout, const std::string &name) const
{
    ...
    OPS_LOG_E(opName_, "Tensor %s only supports layoutQuery %s, but got %s", name.c_str(), oss.str().c_str(),
...
ge::graphStatus SFATilingCheck::CheckLayoutSupport(const SFALayout &actualLayout, const std::string &name) const
{
    ...
    OPS_ERR_IF(std::find(expectLayoutList.begin(), expectLayoutList.end(), actualLayout) == expectLayoutList.end(),
...
{
    const std::vector<std::string> layoutQueryList = {"BSND", "TND"};
    ...
    if (std::find(layoutQueryList.begin(), layoutQueryList.end(), layoutQuery) == layoutQueryList.end()) {
        OPS_LOG_E(opName_, "Layout only supports BSND/TND, but got %s", layoutQuery.c_str());
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus SFATilingCheck::CheckSinglePara() const
{
    if (ge::GRAPH_SUCCESS != CheckSingleParaQuery() || ge::GRAPH_SUCCESS != CheckSingleParaKey() ||
        ge::GRAPH_SUCCESS != CheckSingleParaValue() || ge::GRAPH_SUCCESS != CheckSingleParaSparseIndices() ||
        ge::GRAPH_SUCCESS != CheckSingleParaQueryRope() || ge::GRAPH_SUCCESS != CheckSingleParaKeyRope() ||
        ge::GRAPH_SUCCESS != CheckSingleParaAttenOut() || ge::GRAPH_SUCCESS != CheckSingleParaLayout() ||
...
{
    OPS_ERR_IF(pointer != nullptr, OPS_LOG_E(opName_, "%s should be null", name.c_str()), return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SFATilingCheck::CheckActualSeqLensMulti()
{
    ...
    if (qLayout_ == SFALayout::TND) {
        if (GetActualSeqLenSize(shapeSizeKV, opParamInfo_.actualSeqLengths.tensor, kvLayout_, "actualSeqLengths") !=
            ge::GRAPH_SUCCESS) {
            return ge::GRAPH_FAILED;
        }
        if (GetActualSeqLenSize(shapeSizeQ, opParamInfo_.actualSeqLengthsQ.tensor, qLayout_, "actualSeqLengthsQ") !=
            ge::GRAPH_SUCCESS) {
            return ge::GRAPH_FAILED;
        }
        if (shapeSizeQ == 0 || shapeSizeKV == 0) {
            OPS_LOG_E(opName_,
                      "In TND layout, actualSeqLengthsQ shape size is %u, actualSeqLengths shape size is %u, they "
                      "should  > 0.",
...
{
    SetSFAShapeCompare();
    if (ge::GRAPH_SUCCESS != CheckQAndQRope() || ge::GRAPH_SUCCESS != CheckKV() || ge::GRAPH_SUCCESS != CheckTopK() ||
        ge::GRAPH_SUCCESS != CheckAttenOut() || ge::GRAPH_SUCCESS != CheckActualSeqLensQ() ||
        ge::GRAPH_SUCCESS != CheckActualSeqLens() || ge::GRAPH_SUCCESS != CheckActualSeqLensMulti()) {
        return ge::GRAPH_FAILED;
    }
...
{
    OPS_ERR_IF(n2Size_ != 1, OPS_LOG_E(opName_, "kv_head_num should be 1, but got %u", n2Size_),
               return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(qkHeadDim_ != 512, OPS_LOG_E(opName_, "qk_head_dim only support 512, but got %u", qkHeadDim_),
               return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(ropeHeadDim_ != 64, OPS_LOG_E(opName_, "rope_head_dim should be 64, but got %u", ropeHeadDim_),
               return ge::GRAPH_FAILED);

    OPS_ERR_IF(
        qTSize_ > 1024 * 1024 / GetTypeSize(inputQType_),
...
ge::graphStatus SFATilingCheck::CheckFeatureMlaNoquantPa() const
{
    ...
    OPS_ERR_IF(kvLayout_ != SFALayout::PA_BSND,
               OPS_LOG_E(opName_,
                         "SparseFlashAttention is enabled, only supports "
                         "key/value's layout is PA_BSND, but now is %s",
...
ge::graphStatus SFATilingCheck::Process()
{
    ...
    if (CheckSinglePara() != ge::GRAPH_SUCCESS || CheckParaExistence() != ge::GRAPH_SUCCESS ||
        CheckMuiltPara() != ge::GRAPH_SUCCESS || CheckFeature() != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SFAInfoParser::CheckRequiredInOutExistence() const
{
    ...
    OPS_ERR_IF(opParamInfo_.sparseIndices.shape == nullptr,
               OPS_LOG_E(opName_, "Shape of tensor sparseIndices is nullptr"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(opParamInfo_.sparseIndices.desc == nullptr,
               OPS_LOG_E(opName_, "Desc of tensor sparseIndices is nullptr"), return ge::GRAPH_FAILED);
...
ge::graphStatus SFAInfoParser::GetNpuInfo()
{
    ...
    OPS_ERR_IF(platformInfo_ == nullptr, OPS_REPORT_VECTOR_INNER_ERR(opName_, "GetPlatformInfo is nullptr."),
               return ge::GRAPH_FAILED);

    auto ascendcPlatform = platform_ascendc::PlatformAscendC(platformInfo_);
    uint32_t aivNum = ascendcPlatform.GetCoreNumAiv();
    uint32_t aicNum = ascendcPlatform.GetCoreNumAic();
    OPS_ERR_IF(aicNum == 0 || aivNum == 0, OPS_REPORT_VECTOR_INNER_ERR(opName_, "num of core obtained is 0."),
               return GRAPH_FAILED);
...
ge::graphStatus SFAInfoParser::GetMaxBlockNumPerBatch()
{
    if (opParamInfo_.blockTable.tensor == nullptr) {
        OPS_LOG_E(opName_, "the layout_kv is %u, blockTable must be provided.",
                  SFALayoutToSerialString(kvLayout_).c_str());
        return ge::GRAPH_FAILED;
    }
