...
def gather_kv(
    ...
    for sparse_id in sparse_indices:
        ...
        end_idx = (
            ...
            if begin_idx + sparse_block_size <= cur_actual_seq_lengths_kv
            else cur_actual_seq_lengths_kv
        )
...
def mask(res, cur_actual_seq_q, cur_actual_seq, topk_indices, s1_idx, sparse_blocksize):
    ...
    for _, sparse_id in enumerate(topk_indices):
        ...
        block_len = (
            sparse_blocksize
            if sparse_id != sparse_tail_idx - 1
            else sparse_tail_seq_len
        )
...
def cpu_sparse_flash_attention(
    ...
    q_bnsd_tensor = np.transpose(
        np.concatenate([query, query_rope], axis=-1), axes=(0, 2, 1, 3)
    )
    k_bnsd_tensor = np.transpose(
        np.concatenate([key, key_rope], axis=-1), axes=(0, 2, 1, 3)
    )
...
class TestCustomSFA(TestCase):
    def test_sfa_eager(self):
        ...
        query = torch.tensor(np.random.uniform(-10, 10, (4, 1, 128, 512))).to(
            torch.float16
        )
        key = torch.tensor(np.random.uniform(-5, 10, (4, 8192, 1, 512))).to(
            torch.float16
        )
        value = key.clone()
        sparse_indices = torch.tensor(np.random.uniform(0, 16, (4, 1, 1, 2048))).to(
            torch.int32
        )
        query_rope = torch.tensor(np.random.uniform(-10, 10, (4, 1, 128, 64))).to(
            torch.float16
        )
        key_rope = torch.tensor(np.random.uniform(-10, 10, (4, 8192, 1, 64))).to(
            torch.float16
        )
        act_seq_q = [1] * 4
        act_seq_kv = [4096] * 4
    ...
    def test_sfa_graph(self):
        ...
        class Network(nn.Module):
            ...
            def forward(
                ...
                out1 = torch_npu.npu_sparse_flash_attention(
                    query,
                    key,
                    value,
                    sparse_indices,
