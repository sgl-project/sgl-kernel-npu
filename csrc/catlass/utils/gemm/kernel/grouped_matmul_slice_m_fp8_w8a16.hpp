...
public:
    ...
    struct Params {
        ...
        Params(
            ...
            problemCount(problemCount_), ptrGroupList(ptrGroupList_),
    ...
    static Params ToUnderlyingArguments(const Arguments &args, uint8_t * workspace)
    {
        Params params{
            args.problemShape,
            args.problemCount, args.ptrGroupList,
    ...
    void operator()<AscendC::AIV>(Params const &params)
    {
        ...
        AscendC::GlobalTensor<ElementB> gmBlockB;
        gmBlockB.SetGlobalBuffer(reinterpret_cast<__gm__ ElementB *>(params.ptrWorkspace) + gmOffsetB);
        // fp8反量化分组scale scale形状是((K + 127) // 128, (N + 127) // 128)
        ...
        for (uint32_t groupIdx = 0; groupIdx < params.problemCount; ++groupIdx) {
            ...
            gmScaleFull.SetGlobalBuffer(reinterpret_cast<__gm__ ElementScale *>(params.ptrPerGroupScale + gmGroupOffsetS * sizeof(ElementScale)));
            ...
            for (uint32_t loopIdx = startLoopIdx; loopIdx < coreLoops; loopIdx += coreNum) {
                ...
                MatrixCoord offsetScaleCoord{scaleRowStart, scaleColStart}; // 这个块对应的scales矩阵在全局scale矩阵中的偏移量
                MatrixCoord scaleTileShape{scaleTileRows, scaleTileCols}; // 这个块对应的scales矩阵的行和列数

                auto gmBlockScale = gmScaleFull[params.layoutScale.GetOffset(offsetScaleCoord)]; // 这个块对应的scales矩阵在全局scale矩阵中的偏移位置地址
                auto layoutBlockScale = params.layoutScale.GetTileLayout(scaleTileShape); // 四个成员 前两个是tile尺寸 第三个为所在的全局矩阵的列尺寸 (行主序)
            
                // Compute block-scoped matrix multiply-add
                blockMmad.Prologue(
                    gmPrologueB[gmOffsetB], layoutBlockPrologueB,
                    gmBlockB, layoutBlockB,
            ...
            gmGroupOffsetS += ((inGroupProblemShape.k() + 127) / groupSize)  * ((inGroupProblemShape.n() + 127) / groupSize);
            // AscendC::printf("groupIdx is %d, gmGroupOffsetB is %d, %d\n", groupIdx, gmGroupOffsetB, gmGroupOffsetS);
            // AscendC::DumpTensor(gmScaleFull, groupIdx, 16);
            startCoreIdx = (startCoreIdx + coreLoops) % coreNum;
        }
    }
...
#endif // CATLASS_GEMM_KERNEL_GROUPED_MATMUL_M_FP8_HPP
