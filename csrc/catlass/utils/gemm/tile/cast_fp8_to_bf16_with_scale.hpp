...
struct TileCastFp8ToBf16WithScaleDequant {
    ...
    TileCastFp8ToBf16WithScaleDequant(Arch::Resource<ArchTag> const &resource, Params const &params_) : params(params_)
    {
        ...
        for (uint32_t i = 0; i < BUFFER_NUM; i++) {
            outputBuffer[i] = (resource.ubBuf.template GetBufferByByte<ElementSrc>(bufferOffset * sizeof(ElementSrc))).template ReinterpretCast<bfloat16_t>();
            bufferOffset += COMPUTE_LENGTH * 2;
        }
        for (uint32_t i = 0; i < BUFFER_NUM; i++) {
            workspace[i] = (resource.ubBuf.template GetBufferByByte<ElementSrc>(bufferOffset * sizeof(ElementSrc))).template ReinterpretCast<half>();
            bufferOffset += COMPUTE_LENGTH * 2;
        }
        for (uint32_t i = 0; i < BUFFER_NUM; i++) {
            tmpFp32[i] = (resource.ubBuf.template GetBufferByByte<ElementSrc>(bufferOffset * sizeof(ElementSrc))).template ReinterpretCast<float>();
            bufferOffset += COMPUTE_LENGTH * 4;
        }

        InitLocalMaskVec(resource, bufferOffset);

        scaleCacheBuffer = (resource.ubBuf.template GetBufferByByte<int8_t>(bufferOffset * sizeof(int8_t))).template ReinterpretCast<float>();
    ...
    )
    {
        ...
        if constexpr (std::is_same_v<LayoutSrc, layout::RowMajor>) {
            tilesNum = layoutSrc.shape(0); // L1 K 256
            tileLen = layoutSrc.shape(1); // L1 N 256
        ...
        for (int ldx = 0; ldx < static_cast<int>(totalLoops); ldx++) {
            ...
            AscendC::DataCopyPad(inputBuffer[pingpong], gmSrc[srcProcessOffset],
                ...
                for (uint32_t g_row = 0; g_row < groupNuminRow; g_row++) {
                    ...
                    for (uint32_t g_col = 0; g_col < groupNuminCol; g_col++) {
                        ...
                        AscendC::Muls<float>(
                            ...
                            {1,1,(uint8_t)rowStrideDb,(uint8_t)rowStrideDb});
                        AscendC::Muls<float>(
                            tmpFp32[pingpong][fp32Offset + 64],
                            tmpFp32[pingpong][fp32Offset + 64],
                            scale,
                            mask,
                            rowNuminGroup,
    ...
    CATLASS_DEVICE
    void loadAllTileScales(uint32_t scaleGroupNumk, uint32_t scaleGroupNumN, uint32_t scaleStride, AscendC::GlobalTensor<ElementScale> const gmScaleB) {
