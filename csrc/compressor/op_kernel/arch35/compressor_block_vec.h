    ...
    uint32_t globalSeqIdx, uint32_t dDealSize)
{
    ...
    if constexpr (COMP::cacheMode == CACHE_MODE::EXPLICIT) {
        // EXPLICIT commits every ratio from CommitState after the kernel-wide
        // SyncAll (see ComputeVec1). A row is `pos % ring_size`, so an inline
        // write here would clobber the (pos - ring_size) row this same slice's
        // ReadState still has to read.
    } else {
                                                              ...
                                                              uint32_t dDealSize, uint32_t stateIdx)
{
    ...
    if constexpr (COMP::cacheMode == CACHE_MODE::EXPLICIT) {
        ...
        // C4 (OVERLAP) persists every row instead. A call that starts mid-sequence
        // (a cache hit's chunked prefill) reads its first chunk's history from
        // the ring, and a tail-only policy leaves precisely those rows unwritten,
        // so the hit would compress from stale state while a full prefill reads
        // the same window from its own x.
    }
