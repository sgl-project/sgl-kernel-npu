                                                              ...
                                                              uint32_t dDealSize, uint32_t stateIdx)
{
    ...
    if constexpr (COMP::cacheMode == CACHE_MODE::EXPLICIT) {
        if constexpr (COMP::coff == COFF::OVERLAP) {
            // C4 persists every position it is handed. With a ring wider than the
            // overlap window a chunk's own rows are (coff-1)*cmpRatio rows away
            // from the window it reads, so writing them all cannot clobber it,
            // and every position that a later resume reads stays in the ring.
        } else {
