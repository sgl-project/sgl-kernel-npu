...
namespace sglang::KvQuantSASHost {
...
public:
    explicit KvQuantSparseAttnSharedkv(const char *name) : OpDef(name) {
        Input("q").ParamType(REQUIRED).DataTypeList({ge::DT_BF16}).FormatList({ge::FORMAT_ND});
        Input("ori_kv").ParamType(OPTIONAL).DataTypeList({ge::DT_FLOAT8_E4M3FN}).FormatList({ge::FORMAT_ND});
        Input("cmp_kv").ParamType(OPTIONAL).DataTypeList({ge::DT_FLOAT8_E4M3FN}).FormatList({ge::FORMAT_ND});
        for (const char *name : {"ori_sparse_indices", "cmp_sparse_indices", "ori_block_table",
                                 "cmp_block_table", "cu_seqlens_q", "cu_seqlens_ori_kv",
                                 "cu_seqlens_cmp_kv", "seqused_q", "seqused_kv", "metadata"}) {
            Input(name).ParamType(OPTIONAL).DataTypeList({ge::DT_INT32}).FormatList({ge::FORMAT_ND});
        }
        Input("sinks").ParamType(OPTIONAL).DataTypeList({ge::DT_FLOAT}).FormatList({ge::FORMAT_ND});
        Output("attn_out").ParamType(REQUIRED).DataTypeList({ge::DT_BF16}).FormatList({ge::FORMAT_ND});
