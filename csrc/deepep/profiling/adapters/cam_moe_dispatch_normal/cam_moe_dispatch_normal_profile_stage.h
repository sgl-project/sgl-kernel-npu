#ifndef DEEPEP_PROFILING_ADAPTERS_CAM_MOE_DISPATCH_NORMAL_PROFILE_STAGE_H
#define DEEPEP_PROFILING_ADAPTERS_CAM_MOE_DISPATCH_NORMAL_PROFILE_STAGE_H
...
// stageId 顺序必须与 kernel 侧（ops/op_kernel/profiling/adapters/cam_moe_dispatch_normal/...）保持一致。
// 打点方式：
//   - 发送阶段（InputToShare / SetStatus）与接收等待阶段（WaitStatus）：对函数整体计时，每轮一条；
//   - ShareToOutputLongSeq 拆分为 ShareToOutputCommon（公共准备，每轮一条）+ ShareToOutputExpert（每 expert 一条）；
...
#endif  // DEEPEP_PROFILING_ADAPTERS_CAM_MOE_DISPATCH_NORMAL_PROFILE_STAGE_H
