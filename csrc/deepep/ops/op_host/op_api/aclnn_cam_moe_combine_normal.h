...
__attribute__((visibility("default"))) aclnnStatus aclnnCamMoeCombineNormalGetWorkspaceSize(
    ...
    const aclTensor *recvTopkWeights, const aclTensor *tokenIdx, const aclTensor *tokenIdxMap,
    const aclTensor *tpRecvCountsOptional, char *epGroupName, int64_t epWorldSize, int64_t epRankId,
    char *tpGroupNameOptional, int64_t tpWorldSize, int64_t tpRankId, int64_t moeExpertNum, int64_t realMaxBs,
    int32_t round, int32_t per_round_tokens, const aclTensor *out, const aclTensor *sendCostStats,
