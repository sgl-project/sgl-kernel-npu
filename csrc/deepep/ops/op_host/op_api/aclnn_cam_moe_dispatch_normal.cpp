...
aclnnStatus aclnnCamMoeDispatchNormalGetWorkspaceSize(
    ...
    const aclTensor *recvOffset, const aclTensor *recvCount,
    const aclTensor *expert_global_offset, const aclTensor *srcrank_in_expert_offset, const aclTensor *r_in_srcrank_offset,
	char *groupEp, int64_t epWorldSize, int64_t epRankId, char *groupTpOptional, int64_t tpWorldSize, int64_t tpRankId,
	int64_t moeExpertNum, int64_t quantMode,
    int64_t realMaxBs, int64_t globalBs, int32_t round, int32_t perRoundTokens, const aclTensor *recvX,
	const aclTensor *recvXScales, const aclTensor *assistInfoForCombine,
    const aclTensor *waitRecvCostStats, uint64_t *workspaceSize, aclOpExecutor **executor)
{
    return aclnnInnerCamMoeDispatchNormalGetWorkspaceSize(
        ...
        expert_global_offset, srcrank_in_expert_offset, r_in_srcrank_offset,
		groupEp, epWorldSize, epRankId, groupTpOptional,
        tpWorldSize, tpRankId, moeExpertNum, quantMode,
		realMaxBs, globalBs, round, perRoundTokens, recvX, recvXScales, assistInfoForCombine,
