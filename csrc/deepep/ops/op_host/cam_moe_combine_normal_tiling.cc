...
static ge::graphStatus GetAttrAndSetTilingData(
    ...
    CamMoeCombineNormalTilingData &tilingData,
    const char *nodeName,
    std::string &groupEp,
    std::string &groupTp)
{
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE(nodeName, "groupEp is invalid."),
    ...
    if (*tpWorldSizePtr > 1) {
        OP_TILING_CHECK(
            ...
            return ge::GRAPH_FAILED);
        OP_TILING_CHECK(
            (groupTpPtr == nullptr) || (strnlen(groupTpPtr, MAX_GROUP_NAME_LENGTH) == 0) ||
    ...
    OP_TILING_CHECK(
        (curDispatchStatusNum > DISPATCH_STATUS_MAX_SUPPORT_NUM),
        OP_LOGE(
            nodeName,
...
static bool CheckInputTensorDim(gert::TilingContext *context, const char *nodeName)
{
    ...
    OP_TILING_CHECK(
        recvXStorageShape->GetStorageShape().GetDimNum() != TWO_DIMS,
        OP_LOGE(
            nodeName, "recvX must be 2-dimension, but got %lu dim", recvXStorageShape->GetStorageShape().GetDimNum()),
        return false);
    ...
    OP_TILING_CHECK(
        tokenSrcInfoStorageShape == nullptr, OP_LOGE(nodeName, "tokenSrcInfoForCombine is null."), return false);
    OP_TILING_CHECK(
        tokenSrcInfoStorageShape->GetStorageShape().GetDimNum() != ONE_DIM,
        OP_LOGE(
            nodeName,
            "tokenSrcInfoForCombine must be 1-dimension, but got %lu dim",
    ...
    OP_TILING_CHECK(
        topkWeightsStorageShape->GetStorageShape().GetDimNum() != TWO_DIMS,
        OP_LOGE(
            nodeName,
            "topkWeights must be 2-dimension, but got %lu dim",
...
static bool CheckOptionalInputTensorDim(gert::TilingContext *context, const char *nodeName)
{
    ...
    OP_TILING_CHECK(
        tpRecvCountsStorageShape->GetStorageShape().GetDimNum() != ONE_DIM,
        OP_LOGE(
            nodeName,
            "tpRecvCounts must be 1-dimension, but got %lu dim",
...
static bool CheckTensorDataType(gert::TilingContext *context, const char *nodeName)
{
    ...
    OP_TILING_CHECK(
        (recvXDesc->GetDataType() != ge::DT_BF16) && (recvXDesc->GetDataType() != ge::DT_FLOAT16),
    ...
    OP_TILING_CHECK(
        (tokenSrcInfoDesc->GetDataType() != ge::DT_INT32),
        OP_LOGE(
            nodeName,
    ...
    OP_TILING_CHECK(
        (xDesc->GetDataType() != recvXDesc->GetDataType()),
...
static bool CheckTensorFormat(gert::TilingContext *context, const char *nodeName)
{
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE(nodeName, "recvXFormat is invalid"),
        return false);
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE(nodeName, "tokenSrcInfoFormat is invalid"),
        return false);
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE(nodeName, "tpRecvCountsFormat is invalid"),
        return false);
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE(nodeName, "topkWeightsFormat is invalid"),
        return false);
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE(nodeName, "xFormat is invalid"),
        return false);

    return true;
}

static bool CheckTensorShape(
    gert::TilingContext *context,
    CamMoeCombineNormalTilingData &tilingData,
    const char *nodeName,
    uint32_t localExpertNum)
{
    ...
    OP_TILING_CHECK(
        (topkWeightsDim1 <= 0) || (topkWeightsDim1 > K_MAX || (topkWeightsDim1 > moeExpertNum)),
        OP_LOGE(
            nodeName,
    ...
    uint32_t &localMoeExpertNum)
{
    ...
    OP_TILING_CHECK(
        (localMoeExpertNum > 1) && (tpWorldSize > 1),
        OP_LOGE(
            nodeName,
            "Cannot support multi-moeExpert %d in a rank when tpWorldSize = %d > 1",
            localMoeExpertNum,
            tpWorldSize),
        return false);
    ...
    int64_t topkWeightsDim0 = topkWeightsStorageShape->GetStorageShape().GetDim(0);
    OP_TILING_CHECK(
        (topkWeightsDim0 <= 0) || (topkWeightsDim0 > BS_UPPER_BOUND),
        OP_LOGE(
            nodeName,
            "Invalid topkWeights dims0(BS) %ld. Should be between [1, %ld].",
            topkWeightsDim0,
            BS_UPPER_BOUND),
        return false);
    ...
    OP_LOGD(
        nodeName,
        "CamMoeCombineNormal *globalBsPtr = %ld, bs = %ld, epWorldSize = %u\n",
        *globalBsPtr,
        topkWeightsDim0,
        epWorldSize);
...
static ge::graphStatus TilingCheckCamMoeCombineNormal(gert::TilingContext *context, const char *nodeName)
{
    // 检查参数shape信息
    OP_TILING_CHECK(
        !CheckTensorDim(context, nodeName), OP_LOGE(nodeName, "param shape is invalid"), return ge::GRAPH_FAILED);
    // 检查参数dataType信息
    OP_TILING_CHECK(
        !CheckTensorDataType(context, nodeName),
        OP_LOGE(nodeName, "param dataType is invalid"),
        return ge::GRAPH_FAILED);
    // 检查参数format信息
    OP_TILING_CHECK(
        !CheckTensorFormat(context, nodeName), OP_LOGE(nodeName, "param Format is invalid"), return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
static ge::graphStatus SetWorkspace(gert::TilingContext *context, const char *nodeName)
{
    ...
    OP_TILING_CHECK(
        workspace == nullptr,
        VECTOR_INNER_ERR_REPORT_TILIING(nodeName, "get workspace failed"),
        return ge::GRAPH_FAILED);
...
static void SetHCommCfg(
    gert::TilingContext *context,
    CamMoeCombineNormalTilingData *tiling,
...
static ge::graphStatus CamMoeCombineNormalA3TilingFuncImpl(gert::TilingContext *context)
{
    ...
    // 获取入参属性
    OP_TILING_CHECK(
        GetAttrAndSetTilingData(context, *tilingData, nodeName, groupEp, groupTp) == ge::GRAPH_FAILED,
        OP_LOGE(nodeName, "Getting attr failed."),
        return ge::GRAPH_FAILED);

    // 检查输入输出的dim、format、dataType
    OP_TILING_CHECK(
        TilingCheckCamMoeCombineNormal(context, nodeName) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "Tiling check params failed"),
        return ge::GRAPH_FAILED);

    // 检查属性的取值是否合法
    OP_TILING_CHECK(
        !CheckAttrs(context, *tilingData, nodeName, localMoeExpertNum),
    ...
    // 检查shape各维度并赋值h,k
    OP_TILING_CHECK(
        !CheckTensorShape(context, *tilingData, nodeName, localMoeExpertNum),
        OP_LOGE(nodeName, "param dim check failed."),
        return ge::GRAPH_FAILED);
    ...
    uint64_t actualSize =
        ((maxBs * tokenNeedSizeDispatch) + (maxBs * tokenNeedSizeCombine * k) + COMBINE_STATE_WIN_OFFSET) *
        DOUBLE_DATA_BUFFER;
    OP_TILING_CHECK(
...
static ge::graphStatus CamMoeCombineNormalTilingFunc(gert::TilingContext *context)
{
    ...
    OP_TILING_CHECK(
        (recvXDesc->GetDataType() == ge::DT_INT32),
        OP_LOGE(nodeName, "recvX dataType is invalid, dataType should be bf16 or float16, but is "),
        return ge::GRAPH_FAILED);
