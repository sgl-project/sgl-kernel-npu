...
template <class ArchTag, class L1Type, class L0Type = void> struct CopyL1ToL0B {
  static_assert(
      ...
      "Unsupporteded copy l1 to l0, can not find the specialization.");
};

////////////////////////////////////////
/// new add gemm
template <class ArchTag, class Element>
struct CopyL1ToL0B<ArchTag, Act::Gemm::GemmType<Element, layout::zZ>,
                   Act::Gemm::GemmType<Element, layout::nZ>> {
  ...
  void operator()(AscendC::LocalTensor<Element> dstTensor,
                  AscendC::LocalTensor<Element> srcTensor, LayoutDst layoutDst,
                  LayoutSrc layoutSrc) {
    ...
    for (uint32_t i = 0; i < layoutDst.shape(3); i++) { // K N
      AscendC::LoadData(dstTensor[i * layoutSrc.stride(1)],
...
struct CopyL1ToL0B<ArchTag, Act::Gemm::GemmType<float, layout::zZ>,
                   Act::Gemm::GemmType<float, layout::nZ>> {
  using Element = float;
  using LayoutDst = layout::nZ;
  using LayoutSrc = layout::zZ;
  ...
  void operator()(AscendC::LocalTensor<Element> dstTensor,
    ...
    loadDataParams.dstFracGap =
        static_cast<uint16_t>(layoutSrc.shape(3) / 2) - 1;
    for (uint32_t i = 0; i < layoutDst.shape(3); i++) { // K N
      AscendC::LoadDataWithTranspose(dstTensor[i * layoutSrc.stride(1)],
...
template <class ArchTag>
struct CopyL1ToL0B<ArchTag, Act::Gemm::GemmType<int8_t, layout::zN>,
                   Act::Gemm::GemmType<int8_t, layout::nZ>> {
  ...
  void operator()(AscendC::LocalTensor<Element> dstTensor,
    ...
    for (uint32_t i = 0; i < KLoops; i++) {
      AscendC::LoadDataWithTranspose(
          dstTensor[i * NRound * KL0Alignment],
...
template <class ArchTag, class Element>
struct CopyL1ToL0B<ArchTag, Act::Gemm::GemmType<Element, layout::zN>,
                   Act::Gemm::GemmType<Element, layout::zN>> {
...
template <class ArchTag, class Element>
struct CopyL1ToL0B<ArchTag, Act::Gemm::GemmType<Element, layout::nZ>,
                   Act::Gemm::GemmType<Element, layout::nN>> {
...
template <class ArchTag, class Element>
struct CopyL1ToL0B<ArchTag, Act::Gemm::GemmType<Element, layout::nN>,
                   Act::Gemm::GemmType<Element, layout::zN>> {
  using LayoutDst = layout::zN;
...
template <class ArchTag>
struct CopyL1ToL0B<ArchTag, Act::Gemm::GemmType<float, layout::nN>,
                   Act::Gemm::GemmType<float, layout::zN>> {
  using LayoutDst = layout::zN;
  using LayoutSrc = layout::nN;
  using Element = float;
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    loadDataParams.startIndex = 0;
    loadDataParams.repeatTimes = static_cast<uint16_t>(
        CeilDiv<C0_NUM_PER_FRACTAL>(layoutDst.orgShape(0)));
    loadDataParams.srcStride = 1;
    loadDataParams.dstGap = 0;
    loadDataParams.dstFracGap =
        CeilDiv<C0_NUM_PER_FRACTAL>(layoutDst.orgShape(0)) - 1;

    for (uint32_t i = 0; i < CeilDiv<2 * ELE_NUM_PER_C0>(layoutDst.orgShape(1));
         i++) {
      AscendC::LoadDataWithTranspose(dstTensor[i * layoutDst.stride(3) * 2],
...
template <class ArchTag>
struct CopyL1ToL0B<ArchTag, Act::Gemm::GemmType<int8_t, layout::nZ>,
                   Act::Gemm::GemmType<int8_t, layout::zN>> {
  using LayoutDst = layout::zN;
  using LayoutSrc = layout::nZ;
  using Element = int8_t;
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    loadDataParams.repeatTimes =
        static_cast<uint16_t>(CeilDiv<ELE_NUM_PER_C0>(layoutDst.orgShape(0)));
    loadDataParams.srcStride = layoutSrc.stride(1) / ELE_NUM_PER_FRACTAL / 2;
    loadDataParams.dstGap = 1;
    loadDataParams.dstFracGap = 0;

    for (uint32_t i = 0; i < CeilDiv<ELE_NUM_PER_C0>(layoutDst.orgShape(1));
         i++) {
      AscendC::LoadDataWithTranspose(dstTensor[i * layoutDst.stride(3)],
...
struct CopyL1ToL0B<ArchTag,
                   Gemm::GemmType<int8_t, layout::zN, AscendC::TPosition::A1>> {
  using Element = int8_t;
  using LayoutDst = layout::nZ;
  using LayoutSrc = layout::zN;
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    loadDataParams.startIndex = 0;
    loadDataParams.repeatTimes =
        static_cast<uint16_t>(CeilDiv<ELE_NUM_PER_C0>(layoutDst.orgShape(1)));
    loadDataParams.srcStride = layoutSrc.stride(3) / ELE_NUM_PER_FRACTAL / 2;
    loadDataParams.dstGap = 1;
    loadDataParams.dstFracGap = 0;

    for (uint32_t i = 0; i < CeilDiv<ELE_NUM_PER_C0>(layoutDst.orgShape(0));
         i++) {
...
template <class ArchTag, class Element>
struct CopyL1ToL0B<
    ArchTag, Gemm::GemmType<Element, layout::zN, AscendC::TPosition::A1>> {
  using LayoutDst = layout::nZ;
  using LayoutSrc = layout::zN;
...
template <class ArchTag, class Element>
struct CopyL1ToL0B<
    ArchTag, Gemm::GemmType<Element, layout::nZ, AscendC::TPosition::A1>> {
  using LayoutDst = layout::nZ;
  using LayoutSrc = layout::nZ;
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    if (layoutSrc.shape(3) == layoutDst.shape(3)) {
      loadDataParams.startIndex = 0;
      loadDataParams.repeatTimes =
          static_cast<uint16_t>(layoutDst.shape(1) * layoutDst.shape(3));
      loadDataParams.srcStride = 1;
      loadDataParams.sid = 0;
      loadDataParams.dstGap = 0;
      loadDataParams.ifTranspose = false;
...
template <class ElementSrc, class ElementDst, class LayoutSrc_,
          class LayoutDst_>
struct TileCopyTla<
    Arch::AtlasA2,
    Tensor<AscendC::LocalTensor<ElementSrc>, LayoutSrc_,
           AscendC::TPosition::A1>,
    Tensor<AscendC::LocalTensor<ElementDst>, LayoutDst_,
  ...
  using TensorDst = Tensor<AscendC::LocalTensor<ElementDst>, LayoutDst,
                           AscendC::TPosition::B2>;
  using TensorSrc = Tensor<AscendC::LocalTensor<ElementSrc>, LayoutSrc,
                           AscendC::TPosition::A1>;
...
struct TileCopyTla<
    Arch::AtlasA2,
    Tensor<AscendC::LocalTensor<int8_t>, LayoutSrc_, AscendC::TPosition::A1>,
  ...
  using LayoutSrc = LayoutSrc_;
  using TensorDst =
      Tensor<AscendC::LocalTensor<Element>, LayoutDst, AscendC::TPosition::B2>;
  using TensorSrc =
      Tensor<AscendC::LocalTensor<Element>, LayoutSrc, AscendC::TPosition::A1>;
  ...
  void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor) {
    ...
    for (uint32_t i = 0; i < dstOuterShapeRow; i++) {
      AscendC::LoadDataWithTranspose(
          dstTensor.data()[i * dstOuterStrideRow],
