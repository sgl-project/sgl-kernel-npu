...
struct CopyL0CToGmQuantMode {
  static_assert(
      ...
      "Unsupporteded copy l0c to gm, can not find the specialization.");
};

// CopyL0CToGm cast fp32 to fp16
template <>
struct CopyL0CToGmQuantMode<Act::Arch::AtlasA2, float, half,
...
template <>
struct CopyL0CToGmQuantMode<Act::Arch::AtlasA2, float, bfloat16_t,
                            ScaleGranularity::NO_QUANT> {
...
template <>
struct CopyL0CToGmQuantMode<Act::Arch::AtlasA2, float, float,
                            ScaleGranularity::NO_QUANT> {
...
template <>
struct CopyL0CToGmQuantMode<Act::Arch::AtlasA2, int32_t, int32_t,
                            ScaleGranularity::NO_QUANT> {
  static constexpr auto VALUE = QuantMode_t::NoQuant;
};
...
template <>
struct CopyL0CToGmQuantMode<Act::Arch::AtlasA2, int32_t, half,
...
struct CopyL0CToGm<Act::Arch::AtlasA2, ElementAccumulator_,
  ...
  static constexpr auto quantPre =
      CopyL0CToGmQuantMode<ArchTag, ElementSrc, ElementDst,
...
struct CopyL0CToGm<Act::Arch::AtlasA2, ElementAccumulator_,
  ...
  ACT_DEVICE
  void operator()(AscendC::GlobalTensor<ElementDst> dstTensor,
...
struct CopyL0CToGm<Act::Arch::AtlasA2, ElementAccumulator_,
  ...
  void operator()(AscendC::GlobalTensor<ElementDst> const &dst,
    ...
    // Call AscendC Fixpipe
    AscendC::Fixpipe<ElementDst, ElementSrc, AscendC::CFG_NZ>(dst, src,
                                                              intriParams);
  }
};
...
template <class TensorSrc_, class ElementDst_, class LayoutDst_,
          bool ReluEnable_>
struct CopyL0CToGmTla<
    Act::Arch::AtlasA2, TensorSrc_,
    Tensor<AscendC::GlobalTensor<ElementDst_>, LayoutDst_,
           AscendC::TPosition::GM>,
    ScaleGranularity::NO_QUANT, ReluEnable_,
    std::enable_if_t<tla::detail::isRowMajor<LayoutDst_>::value>> {
  using ArchTag = Act::Arch::AtlasA2;
  using TensorDst = Tensor<AscendC::GlobalTensor<ElementDst_>, LayoutDst_,
                           AscendC::TPosition::GM>;
  using ElementDst = ElementDst_;
  ...
  void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor,
    ...
    intriParams.srcStride =
        get<1, 1>(srcTensor.stride()) / get<0, 0>(srcTensor.stride());
