...
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::RowMajor>,
                  Gemm::GemmType<Element, layout::zN>> {
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    if (layoutSrc.stride(0) < STRIDE_LIMIT) {
      ...
      for (uint32_t i = 0; i < layoutSrc.shape(0); i++) {
        AscendC::DataCopy(dstTensor[i * ELE_NUM_PER_C0],
                          srcTensor[i * layoutSrc.stride(0)], intriParams);
      }
    }
  }
};
...
template <class ArchTag, class Element>
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::RowMajor>,
                  Gemm::GemmType<Element, layout::zZ>> {
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    if (srcNdStride < STRIDE_LIMIT) {
      ...
      if (remains) {
        ...
        AscendC::DataCopy(dstTensor[ndNum * layoutDst.stride(1)],
                          srcTensor[ndNum * srcNdStride], tailParams);
      }
      ...
      for (uint32_t i = 0; i < ndNum; i++) {
        ...
        AscendC::DataCopy(dstTensor[i * layoutDst.stride(1)],
                          srcTensor[i * srcNdStride], intriParams);
      }
      if (remains) {
        AscendC::Nd2NzParams tailParams;
        tailParams.ndNum = 1;
      ...
      for (uint32_t i = 0; i < layoutSrc.shape(0); i++) {
        ...
        uint32_t offsetDst =
            i * idxR0 * layoutDst.stride(1) + idxInR0 * ELE_NUM_PER_C0;
        uint32_t offsetSrc = i * layoutSrc.stride(0);
        AscendC::DataCopy(dstTensor[offsetDst], srcTensor[offsetSrc],
                          intriParams);
      }
    }
  }
...
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::ColumnMajor>,
                  Gemm::GemmType<Element, layout::nN>> {
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    if (srcNdStride < STRIDE_LIMIT) {
      ...
      if (remains) {
        ...
        AscendC::DataCopy(dstTensor[ndNum * layoutDst.stride(3)],
                          srcTensor[ndNum * srcNdStride], tailParams);
      }
      ...
      for (uint32_t i = 0; i < ndNum; i++) {
        ...
        AscendC::DataCopy(dstTensor[i * layoutDst.stride(3)],
                          srcTensor[i * srcNdStride], intriParams);
      }
      if (remains) {
        AscendC::Nd2NzParams tailParams;
      ...
      for (uint32_t i = 0; i < layoutSrc.shape(1); i++) {
        ...
        uint32_t offsetDst =
            i * idxR0 * layoutDst.stride(3) + idxInR0 * ELE_NUM_PER_C0;
...
template <class ArchTag, class Element>
struct CopyGmToL1<ArchTag, Gemm::GemmType<Element, layout::ColumnMajor>,
                  Gemm::GemmType<Element, layout::nZ>> {
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    if (layoutSrc.stride(1) < STRIDE_LIMIT) {
      ...
      for (uint32_t i = 0; i < layoutSrc.shape(1); i++) {
        AscendC::DataCopy(dstTensor[i * ELE_NUM_PER_C0],
                          srcTensor[i * layoutSrc.stride(1)], intriParams);
      }
    }
  }
};
...
struct CopyGmToL1<Arch::AtlasA2, Gemm::GemmType<Element, layout::RowMajor>> {
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    if (srcNdMatrixStride < STRIDE_LIMIT) {
      ...
      for (uint32_t i = 0; i < ndNum; i++) {
        AscendC::DataCopy(dstTensor[i * ELE_NUM_PER_C0],
                          srcTensor[i * srcNdMatrixStride], intriParams);
      }
    }
  }
};
...
struct CopyGmToL1<Arch::AtlasA2,
                  Gemm::GemmType<Element, layout::PaddingRowMajor>> {
  using LayoutDst = layout::zN;
...
template <class Element>
struct CopyGmToL1<Arch::AtlasA2,
                  Gemm::GemmType<Element, layout::PaddingColumnMajor>> {
  using LayoutDst = layout::nZ;
...
struct CopyGmToL1<
  ...
  void operator()(AscendC::LocalTensor<Element> const &dstTensor,
    ...
    if ((layoutSrc.shape(1) == layoutSrc.stride(0)) &&
        (layoutDst.shape(1) == layoutDst.stride(0))) {
      ...
      for (uint32_t i = 0; i < rLoops; ++i) {
        uint32_t rActual =
            (i < rLoops - 1) ? MAX_REPEAT : rows - i * MAX_REPEAT;
        AscendC::DataCopyParams dataCopyParams(rActual, cols / ELE_NUM_PER_BLK,
                                               srcStride, dstStride);
        DataCopy(dstTensor[i * MAX_REPEAT * layoutDst.stride(0)],
                 srcTensor[i * MAX_REPEAT * layoutSrc.stride(0)],
                 dataCopyParams);
      }
    } else {
      for (uint32_t i = 0; i < rows; ++i) {
        DataCopy(dstTensor[i * layoutDst.stride(0)],
                 srcTensor[i * layoutSrc.stride(0)], cols);
      }
    }
  }
};
...
struct TileCopyTla<
    Arch::AtlasA2,
    Tensor<AscendC::GlobalTensor<ElementSrc>, LayoutSrc_,
  ...
  using TensorDst = Tensor<AscendC::LocalTensor<ElementDst>, LayoutDst,
                           AscendC::TPosition::A1>;
  using TensorSrc = Tensor<AscendC::GlobalTensor<ElementSrc>, LayoutSrc,
                           AscendC::TPosition::GM>;
  ...
  void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor) {
    ...
    if (srcDValue < STRIDE_LIMIT) {
      ...
      for (uint32_t i = 0; i < nValue; i++) {
        AscendC::DataCopy(dstTensor.data()[i * ELE_NUM_PER_C0],
                          srcTensor.data()[i * srcDValue], intriParams);
      }
    }
  }
};
