...
namespace MoeDistributeCombineImpl {
                                              ...
                                              const uint8_t expertLocalId = 0U)
        {
            if (domain == EP_DOMAIN) {
                ...
                return (GM_ADDR)((tpRankId_ == rankId) ? tpWinContext_->localWindowsIn :
                                 ((HcclRankRelationResV2 *)
                                 (tpWinContext_->remoteRes[rankId].nextDevicePtr))->windowsIn) +
    ...
    template <TemplateMC2TypeClass>
    __aicore__ inline void CamMoeDistributeCombine<TemplateMC2TypeFunc>::Init(GM_ADDR expandX, GM_ADDR expertIds,
                                                                              GM_ADDR expandIdx, GM_ADDR epSendCount,
                                                                              GM_ADDR tpSendCount, GM_ADDR scales,
                                                                              GM_ADDR XOut, GM_ADDR workspaceGM,
                                                                              TPipe *pipe,
                                                                              const FusedDeepMoeTilingData *tilingData)
    {
        tpipe_ = pipe;
        ...
        expertPerSizeOnWin_ = static_cast<uint64_t>(axisMaxBs_) * static_cast<uint64_t>(axisH_)\
                            * static_cast<uint64_t>(sizeof(ExpandXType));
        winDataSizeOffset_ = static_cast<uint64_t>(dataState_) * static_cast<uint64_t>(moeSendNum_)\
                            * expertPerSizeOnWin_;
...
// 2 -> win wait syncall gm -> ub win ->gm add -> alltoall
            ...
            uint32_t tokenNumLoop, uint32_t srcStartTokenIdx, uint32_t ep, uint32_t expertIdx)
    {
        ...
        if ((isShardExpert_) && (ep < sharedExpertRankNum_)) {
            rankGM = GetWinAddrByRankId(epRankId_, EP_DOMAIN, expertIdx)\
                    + ep * moeExpertPerRankNum_ * expertPerSizeOnWin_;
        }
        rankWindow_.SetGlobalBuffer((__gm__ ExpandXType *)rankGM);
    ...
    __aicore__ inline void CamMoeDistributeCombine<TemplateMC2TypeFunc>::LocalWindowCopy()
    {
        ...
        for (uint32_t tokenIndex = beginIndex; tokenIndex < endIndex; tokenIndex++) {
            ...
            for (uint32_t i = 0; i < axisK_; i++) {
                ...
                GM_ADDR wAddr = (__gm__ uint8_t *)(epWindowGM_) + expertPerSizeOnWin_ * moeExpertPerRankNum_ *
                                                                  sharedExpertRankNum_
                                                                  + expertPerSizeOnWin_ * moeExpert
                                                                  + indexCountsLocal.GetValue(index) *
                                                                  axisHExpandXTypeSize_
            ...
            if (sharedExpertRankNum_ > 0U) {
                ...
                __gm__ ExpandXType* shareAddr = (__gm__ ExpandXType *)
                        (epWindowGM_ + moeOnShareRank * expertPerSizeOnWin_ * moeExpertPerRankNum_)
