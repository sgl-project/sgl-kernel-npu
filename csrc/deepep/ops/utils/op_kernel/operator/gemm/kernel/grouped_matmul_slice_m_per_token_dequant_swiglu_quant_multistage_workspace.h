...
#define GET_WIND_STATE_ADDR_BY_RANK_ID(rankId) \
                (((epRankId == rankId) ? ((GM_ADDR)(winContext_->localWindowsExp)) : \
...
#define GET_WIND_ADDR_BY_RANK_ID(rankId) \
                (((epRankId == rankId) ? ((GM_ADDR)(winContext_->localWindowsIn)) : \
...
private:
   ...
   __aicore__ inline static void
    EncreaseSyncFlag(__gm__ uint8_t* flagAddr, uint8_t idx)
    {
        // flag++，类似set flag
    ...
    __aicore__ inline static void
    CheckSyncFlag(__gm__ uint8_t* flagAddr, uint8_t idx, uint32_t target)
    {
        //  查看flag，类似wait flag
        AscendC::PipeBarrier<PIPE_ALL>();
        AscendC::GlobalTensor<uint8_t> global;
...
public:
    ...
    struct Params {
        ...
        Params(
            ...
            GM_ADDR gmX_, GM_ADDR debugGm_, GM_ADDR gmexpertIds_,
            GM_ADDR gmExpandIdx_, GM_ADDR gmEpSendCount_, GM_ADDR gmResvered_,
            ...
            gmX(gmX_), debugGm(debugGm_), gmexpertIds(gmexpertIds_),
            gmExpandIdx(gmExpandIdx_), gmEpSendCount(gmEpSendCount_), gmResvered(gmResvered_),
    ...
    void operator()<AscendC::AIC>(Params const &params)
    {
        ...
        AscendC::DataCacheCleanAndInvalid<int32_t, AscendC::CacheLine::SINGLE_CACHE_LINE,
                    AscendC::DcciDst::CACHELINE_OUT>(selfDataStatusTensor[aicStateGlobalCoreIdx * UB_ALIGN]);
        __asm__ __volatile__("");
        ...
        for (uint32_t groupIdx = 0; groupIdx < localExpertNum; ++groupIdx) {
            ...
            uint32_t startLoopIdx = ((aicIdx < startCoreIdx) ? (aicIdx + aicNum) : aicIdx)
                                    - startCoreIdx;
    ...
    void CalExpandxIdx(int32_t dstExpertId, uint32_t tokenIndex, int32_t &curExpertCnt, int64_t ubOffset)
    {
        ...
        AscendC::LocalTensor<int32_t> dstExpIdTensor_ = (resource.ubBuf.template
                                                                GetBufferByByte<int32_t>(ubOffset));
        subUbOffset += LOOP_TMP_SIZE;
        AscendC::LocalTensor<int32_t> subExpIdTensor_ = (resource.ubBuf.template
                                                                GetBufferByByte<int32_t>(ubOffset));
        subUbOffset += LOOP_TMP_SIZE;
        AscendC::LocalTensor<float> workLocalTensor_ = (resource.ubBuf.template
                                                                    GetBufferByByte<float>(ubOffset));
        subUbOffset += LOOP_TMP_SIZE;
    ...
    void CalAndSendTokenCount()
    {
        ...
        AscendC::Duplicate(statusTensor_, (int32_t)0, expertCntUp * INT32_COUNT_PER_BLOCK); // 先清零再赋值，清零一定要做
        if (state == 0) {
        ...
        for (uint32_t rankIndex = startExpertId; rankIndex < endExpertId; ++rankIndex) {
            ...
            if (moeExpertNumPerRank > 1 && (rankIndex >= sharedExpertRankNum)) {
                dstRankId = ((rankIndex - sharedExpertRankNum) / moeExpertNumPerRank
                                    + sharedExpertRankNum);
                offset = (epRankId + (rankIndex - sharedExpertRankNum)
                                    % moeExpertNumPerRank * epRankSize) * stateOffset;
            }
    ...
    CATLASS_DEVICE
    void QuantToken(AscendC::LocalTensor<XType> &xInTensor, AscendC::LocalTensor<int8_t> &yInt8Tensor,
                int64_t ubOffset)
    {
        ...
        AscendC::PipeBarrier<PIPE_V>();
        AscendC::Cast(yInt8Tensor,
                                    yHalfTensor, AscendC::RoundMode::CAST_TRUNC, tokenLength);
    }
    ...
    void SendToShareExprt(GM_ADDR gmX, GM_ADDR gmX1, GM_ADDR gmX1Scale)
    {
        ...
        for (uint32_t tokenIndex = startTokenId; tokenIndex < endTokenId; ++tokenIndex) {
            ...
            dstWinGMTensor.SetGlobalBuffer((__gm__ int8_t*)(GET_WIND_ADDR_BY_RANK_ID(moeOnShareRank)
                                + expertPerSizeOnWin * epRankId));
            ...
            if (isShareExpert) {
                ...
                AscendC::DataCopy(expandXOutGlobal[tokenIndex * tokenLength],
                                                                yInt8Tensor[index], tokenLength);
                ...
                // 怀疑有时序问题，所以分开发送
                AscendC::DataCopy(dstWinGMTensor[(tokenIndex - preCnt) * axisHCommu],
                                                                        yInt8Tensor[index], tokenLength);
                AscendC::PipeBarrier<PIPE_MTE3>();
    ...
    void SendToMoeExprt(GM_ADDR gmX, GM_ADDR gmExpandIdx)
    {
        ...
        for (uint32_t sendGroupIndex = 0; sendGroupIndex < moeExpertNumPerRank; ++sendGroupIndex) {
            for (uint32_t tokenIndex = startTokenId; tokenIndex < endTokenId; ++tokenIndex) {
                ...
                uint32_t tempRankId = dstExpertId / moeExpertNumPerRank
                                        + sharedExpertRankNum;
                GM_ADDR rankGM = (__gm__ uint8_t*)(GET_WIND_ADDR_BY_RANK_ID(tempRankId) + (expertPerSizeOnWin
                                                        * (epRankId * moeExpertNumPerRank
                ...
                AscendC::WaitFlag<AscendC::HardEvent::V_MTE2>(eventId);
                AscendC::WaitFlag<AscendC::HardEvent::MTE3_MTE2>(eventId);
                AscendC::DataCopy(xInTensor[index],
                                        srcWinGMTensor[tokenIndex / axisK_ * tokenLength], tokenLength);
                AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(eventId);
                AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(eventId);
                QuantToken(xInTensor[index], yInt8Tensor[index], ubOffset);
                AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(eventId);
                ...
                AscendC::PipeBarrier<PIPE_MTE3>();
                AscendC::DataCopy(dstWinGMTensor[tokenLength],
                                                        yInt8Tensor[index][tokenLength], scaleParamPad);
                AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>(eventId);
                AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(eventId);
            }
        ...
        AscendC::DataCopyExtParams expertIdsCntParams = {1U,
                                    static_cast<uint32_t>(sendTokenNum * sizeof(uint32_t)), 0U, 0U, 0U};
        AscendC::SetFlag<AscendC::HardEvent::S_MTE3>(0);
        AscendC::WaitFlag<AscendC::HardEvent::S_MTE3>(0);
    ...
    CATLASS_DEVICE
    void SendCoreFunc(GM_ADDR gmX, GM_ADDR gmExpertIds, GM_ADDR gmX1, GM_ADDR gmX1Scale, GM_ADDR gmExpandIdx)
    {
        ubOffset = 0;
    ...
    void RecvCount(int64_t ubOffset)
    {
        ...
        AscendC::LocalTensor<uint32_t> gatherTmpTensor = (resource.ubBuf.template
                                                                            GetBufferByByte<uint32_t>(subUbOffset));
        subUbOffset += CEIL_UP(UB_BLOCK_SIZE);
        ...
        AscendC::DataCopyParams intriParams{static_cast<uint16_t>(recStatusNumPerCore), 1,
                                    static_cast<uint16_t>(15), 0}; // srcStride为15个block
    ...
    void GetCumSum(int32_t startRankId, int32_t recvExpertNum, int64_t ubOffset)
    {
        ...
        if (isShareExpert) {
            for (uint32_t curSatatusExpId = 0; curSatatusExpId < sharedExpertRankNum; ++curSatatusExpId) {
                int32_t curExpertCnt = (curSatatusExpId + 1 + epRankId)
                                            * axisBS / sharedExpertRankNum
                                        - (curSatatusExpId + epRankId)
                                            * axisBS / sharedExpertRankNum;
        ...
        AscendC::PipeBarrier<PIPE_V>();
        AscendC::ReduceSum<float>(gatherMaskOutTensor, gatherMaskOutTensor,
                        workLocalTensor, (startRankId + 1) <= recvExpertNum ? (startRankId + 1) : recvExpertNum);
        AscendC::SetFlag<AscendC::HardEvent::V_S>(0);
        AscendC::WaitFlag<AscendC::HardEvent::V_S>(0);
    }
                    ...
                    uint32_t startRankId, uint32_t endRankId, uint32_t recvRankNumPerCore, int64_t ubOffset)
    {
        ...
        subUbOffset += CEIL_UP(UB_BLOCK_SIZE);
        AscendC::LocalTensor<int32_t> gatherMaskOutCountTensor = (gatherMaskOutTensor.template
        ...
        for (uint32_t index = startRankId; index < endRankId; index++) {
            ...
            if (!isShareExpert && moeExpertNumPerRank > 1) {
                ...
                winOffset = (index % epRankSize) * moeExpertNumPerRank
                            + index / epRankSize;
            }
            GM_ADDR wAddr = (__gm__ uint8_t*)(GET_WIND_ADDR_BY_RANK_ID(epRankId))
                            + winOffset * expertPerSizeOnWin;
            ...
            for (uint32_t j = 0; j < count; j++) {
                ...
                expandXOutGlobal.SetGlobalBuffer((__gm__ int8_t *)(gmX1)
                                                    + (beginIdx + j) * tokenLength, tokenLength);
        ...
        AscendC::DataCopyExtParams dataCopyOutParams = {1U,
                            static_cast<uint32_t>(recvRankNumPerCore * sizeof(int32_t)), 0U, 0U, 0U};
    ...
    void RecvCoreFunc(GM_ADDR gmX1, GM_ADDR gmX1Scale, GM_ADDR gmEpSendCount)
    {
        ...
        if (startRankId < recvExpertNum) {
            ...
            RecvToken(gmX1, gmX1Scale, gmEpSendCount, coreTokenCount,
                        startRankId, endRankId, recvRankNumPerCore, ubOffset);
        }
        ...
        AscendC::DataCopy(groupTokenNumStateTensor[groupId * GROUP_INFO_SIZE],
                            tmpLocalTensor, INT32_COUNT_PER_BLOCK);
                ...
                LayoutPerTokenScale wholeLayoutPerTokenScale, LayoutOutput layoutOutput)
    {
        ...
        auto layoutC = layout::RowMajor{L1TileShape::M * aiCoreGroupNum * WORKSPACE_STAGES, L1TileShape::N};
        {
            ...
            for (uint32_t groupIdx = 0; groupIdx < localExpertNum; ++groupIdx) {
                ...
                EpilogueParams epilogueParams{
                    gmScale + gmGroupOffsetScale, layoutScale,
        ...
        AscendC::DataCopy(softSyncTensor[(compCoreIdx + COMP_AIV_CORE_NUM)
    ...
    void AivInitState()
    {
        ...
        __asm__ __volatile__("");
        AscendC::DataCacheCleanAndInvalid<int32_t, AscendC::CacheLine::SINGLE_CACHE_LINE,
                            AscendC::DcciDst::CACHELINE_OUT>(selfDataStatusTensor[aivStateGlobalCoreIdx * UB_ALIGN]);
        __asm__ __volatile__("");
        ...
        if (isRecvCore && recvCoreIdx == (recvCoreNum - 1)) {
            ...
            expertTokenNumsOutGMTensor_.SetGlobalBuffer((__gm__ int64_t *)(ptrGroupList));
                ...
                for (uint32_t localMoeIndex = 0; localMoeIndex < localExpertNum; ++localMoeIndex) {
                    ...
                    __asm__ __volatile__("");
                    uint32_t tokenNum = sendCountsGlobal.GetValue(
                                    localMoeIndex * epRankSize + epRankSize - 1);
                    ...
                    AscendC::DataCacheCleanAndInvalid<int64_t, AscendC::CacheLine::SINGLE_CACHE_LINE,
                                    AscendC::DcciDst::CACHELINE_OUT>(expertTokenNumsOutGMTensor_[localMoeIndex]);
                    __asm__ __volatile__("");
                }
        }
    }
    ...
    void operator()<AscendC::AIV>(Params const &params)
    {
        ...
        auto gmSwigluOutput = reinterpret_cast<__gm__ float *>(params.ptrWorkspace
                    + sizeof(int32_t) * (L1TileShape::M * aiCoreGroupNum * WORKSPACE_STAGES * L1TileShape::N));
        if (isCompCore) {
