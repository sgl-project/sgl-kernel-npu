                                ...
                                GM_ADDR workspace, TPipe *pipe, const DispatchLayoutTilingData *tilingData)
    {
        ...
        if (coreIdx_ < aivNum_) {
            ...
            if (coreIdx_ < restNum) {
                ...
                topkIdxOffset = restNum * (tempTokens_ + 1) * numTopk_ * sizeof(int64_t)
                                + (coreIdx_ - restNum) * tempTokens_ * numTopk_ * sizeof(int64_t);
                isTokenOffset = restNum * (tempTokens_ + 1) * numRanks_ * sizeof(T)
                                + (coreIdx_ - restNum) * tempTokens_ * numRanks_ * sizeof(T);
            }
