...
private:
    ...
    __aicore__ inline void ReadBufferAndWeightedSum(uint32_t tokenIndex, uint32_t startTokenIndex, int32_t validOffset);
...
__aicore__ inline void CamMoeCombineNormal<TemplateMC2TypeFunc>::ReadBufferFromRemote()
{
    ...
    for (uint32_t tokenIndex = startTokenIndex; tokenIndex < endTokenIndex; tokenIndex++) {
        ...
        ReadBufferAndWeightedSum(tokenIndex, startTokenIndex, validOffset);
    }
}
