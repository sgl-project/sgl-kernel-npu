...
public:
    ...
    struct Params {
        ...
        CATLASS_HOST_DEVICE
        Params(GemmCoord problemShape_, uint32_t problemCount_, GM_ADDR ptrGroupList_, GM_ADDR ptrA_, GM_ADDR ptrB_,
    ...
    void operator()(Params const &params)
    {
        ...
        for (uint32_t groupIdx = 0; groupIdx < params.problemCount; ++groupIdx) {
            ...
            for (uint32_t loopIdx = 0; loopIdx < coreLoops; ++loopIdx) {
                ...
                auto gmPerTokenTile =
                    GetTile(gmPerTokenVector, tla::MakeCoord(totalM + mCoord + firstHalfBlockM * subBlockIdx),
                            tla::MakeShape(halfBlockM));
                auto gmDequantTile =
                    GetTile(gmDequantTensor, tla::MakeCoord(totalM + mCoord + firstHalfBlockM * subBlockIdx, nCoord),
                            tla::MakeShape(halfBlockM, blockN));
                auto layoutGmm = tla::MakeLayout(tla::MakeShape(halfBlockM, blockN),
