...
template <class ArchTag, class TensorSrc, class TensorDst,
          ScaleGranularity DEQUANT_GRANULARITY = ScaleGranularity::NO_QUANT, class Enable = void>
...
struct CopyL0CToGmSparseTla<
    ...
    CATLASS_DEVICE void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor)
    {
        ...
        params.nSize = tla::min<int, int>(tla::get<1, 0>(srcShape) * tla::get<1, 1>(srcShape),
                                          tla::get<1>(dstShape) - tla::get<1>(coord));
        params.mSize = tla::min<int, int>(tla::get<0, 0>(srcShape) * tla::get<0, 1>(srcShape),
                                          tla::get<0>(dstShape) - tla::get<0>(coord));
        ...
        AscendC::Fixpipe<ElementDst, ElementSrc, AscendC::CFG_ROW_MAJOR>(dstTensor.data()[dstOffset],
                                                                         srcTensor.data()[srcOffset], params);
    }
};
