...
struct TreeVisitor : VisitorImpl<ChildOps..., NodeOp> {
    ...
    struct Callbacks : CallbacksImpl {
        ...
        CATLASS_DEVICE auto collect_child_outputs(TensorC const &tensorTile, MatrixCoord const &alignedTileShape,
                                                  MatrixCoord const &globalOffset, tla::seq<Is...>, Args const &...args)
        {
        ...
        CATLASS_DEVICE auto call_parent_with_outputs(TensorC const &tensorTile, MatrixCoord const &alignedTileShape,
                                  ...
                                  MatrixCoord const &globalOffset, Args const &...args)
        {
            ...
            // 访问所有子节点，收集输出
            auto child_outputs = collect_child_outputs<Stage, ArchTag>(tensorTile, alignedTileShape, globalOffset,
                                                                       tla::make_seq<Rm1>{}, args...);

            // 将子节点输出传给父节点
            return call_parent_with_outputs<Stage, ArchTag>(tensorTile, alignedTileShape, globalOffset, child_outputs,
                                                            tla::make_seq<Rm1>{});
        }
    };
