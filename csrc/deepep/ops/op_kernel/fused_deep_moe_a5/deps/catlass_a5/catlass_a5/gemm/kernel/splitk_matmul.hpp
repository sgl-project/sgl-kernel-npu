        ...
        for (uint32_t loopIdx = aivId; loopIdx < loops; loopIdx += aivNum) {
            ...
            if (elementCount < 4294977295U / sizeof(ElementAccumulator)) {
                ...
                for (uint32_t i = 0; i < splitkFactor; ++i) {
                    ...
                    AscendC::DataCopyPad(accumulatorBuffer[bufferIndex][i * RoundUp(actualTileLen, ELE_NUM_ALIGN)],
            ...
            for (uint32_t sliceIdx = 1; sliceIdx < splitkFactor; ++sliceIdx) {
                ...
                AscendC::Add(accumulatorBuffer[bufferIndex][0], accumulatorBuffer[bufferIndex][0],
...
private:
    ...
    static_assert(STAGES * COMPUTE_LENGTH * sizeof(ElementAccumulator) <= ArchTag::UB_SIZE, "Exceeding the UB space!");
};
