...
template <bool PAGED_CACHE_FLAG_, bool ENABLE_UNIT_FLAG_, class L1TileShape_, class L0TileShape_, class ElementA_,
          class ElementB_, class ElementC_, class ElementBias_, class TileCopy_, class TileMmad_>
struct BlockMmadTla<MmadFAIQKMx<Arch::Ascend950, PAGED_CACHE_FLAG_, ENABLE_UNIT_FLAG_>, L1TileShape_, L0TileShape_,
...
public:
    ...
    CATLASS_DEVICE void operator()(TensorA &tensorA, TensorB &tensorB, TensorC &tensorC, TensorMxScaleA &tensorMxScaleA,
                                   TensorMxScaleB &tensorMxScaleB, AscendC::GlobalTensor<int32_t> &blockTable,
                                   Shape &actualShape, int64_t taskId, int32_t blockSize, bool isFirstLoop = false,
                                   bool isLastLoop = false)
    {
        ...
        for (int32_t kIdx = 0; kIdx < kLoops; ++kIdx) {
            ...
            auto tensorL1TileMxScaleA =
                GetTile(tensorL1MxScaleA, tla::MakeCoord(0, kIdx * L0_TILE_K / MX_SCALE_GROUP_NUM),
                        tla::MakeShape(blockM, tileK / MX_SCALE_GROUP_NUM));
            auto tensorTileMxScaleA = GetTile(tensorMxScaleA, tla::MakeCoord(0, kIdx * L0_TILE_K / MX_SCALE_GROUP_NUM),
            ...
            auto tensorL1MxScaleB = tla::MakeTensor(l1MxScaleBTensorList_[l1BListId_], layoutMxScaleBInL1,
                                                    tla::MakeCoord(0, 0), Arch::PositionL1{});
            auto tensorTileMxScaleB = GetTile(tensorMxScaleB, tla::MakeCoord(kIdx * L0_TILE_K / MX_SCALE_GROUP_NUM, 0),
...
private:
    ...
    CATLASS_DEVICE void CopyInL1B(TensorL1B &tensorL1B, TensorB &tensorB, AscendC::GlobalTensor<int32_t> &blockTable,
                                  int32_t tileK, int32_t tileN, int32_t kOffset, int32_t blockSize)
    {
        ...
        if constexpr (PAGED_CACHE_FLAG_) {
            ...
            for (int32_t blockIdx = 0; blockIdx < blockLoops; ++blockIdx) {
                ...
                auto tensorTileB = GetTile(tensorB, tla::MakeCoord(kOffset, idInBlockTable * blockSize),
