...
template <class ArchTag_, bool ENABLE_UNIT_FLAG_, class L1TileShape_, class L0TileShape_, class ElementA_,
          class ElementB_, class ElementC_, class ElementBias_, class TileCopy_>
struct BlockMmadSparseTla<SparseMatmulMultiBlockOnKAxis<ArchTag_, ENABLE_UNIT_FLAG_>, L1TileShape_, L0TileShape_,
                          ElementA_, ElementB_, ElementC_, ElementBias_, TileCopy_> {
public:
        ...
        for (auto mIndex = 0; mIndex < mIter; ++mIndex) {
            for (auto nIndex = 0; nIndex < nIter; ++nIndex) {
                ...
                for (auto kOuterIndex = 0; kOuterIndex < kOuterIter; ++kOuterIndex) {
                    ...
                    auto srcBTlaTensor =
                        GetTile(b, tla::MakeCoord(bL1Row * L1_K / DENSE_MATRIX_B_OFFSET, bL1Col * L0_N),
                    ...
                    for (auto kInnerIndex = 0; kInnerIndex < kInnerIter; ++kInnerIndex) {
                        ...
                        auto l1ATileTensor =
                            tla::MakeTensor(l1ATensor.data(), l1ATensor.layout(),
                                            tla::MakeCoord(aL0Row * 0, aL0Col * L0_K), Arch::PositionL1{});
                ...
                auto l0CSrcTensor = tla::MakeTensor(l0CTlaTensor.data(), l0CTlaTensor.layout(),
                                                    tla::MakeCoord(mIndex * L0_M, nIndex * L0_N), Arch::PositionL0C{});
