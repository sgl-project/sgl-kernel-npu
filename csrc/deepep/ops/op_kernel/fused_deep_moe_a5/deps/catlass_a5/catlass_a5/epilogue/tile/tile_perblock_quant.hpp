...
struct TilePerBlockQuant {
    ...
    static_assert(std::is_same_v<ArchTag, Arch::Ascend950> && (std::is_same_v<ElementSrc, bfloat16_t>) &&
    ...
    __simd_vf__ void perBlockScaleQuant(__ubuf__ ElementSrc *srcUb, __ubuf__ ElementDst *dstUb,
                                        __ubuf__ ElementScale *scaleUb, uint16_t nLoops)
    {
        static constexpr CastTrait ctFp322Fp8 = {RegLayout::ZERO, SatMode::NO_SAT, MaskMergeMode::ZEROING,
                                                 AscendC::RoundMode::CAST_RINT};
        static constexpr CastTrait ctFp322Fp16 = {RegLayout::ZERO, SatMode::NO_SAT, MaskMergeMode::ZEROING,
                                                  AscendC::RoundMode::CAST_RINT};
        ...
        for (uint16_t i = 0; i < nLoops; i++) {
            ...
            StoreAlign<uint8_t, StoreDist::DIST_PACK4_B32>(reinterpret_cast<__ubuf__ uint8_t *>(dstUb) + i * VL_B16,
                                                           (RegTensor<uint8_t> &)srcRegZero, pregFullB8);
        ...
        StoreAlign<uint32_t, StoreDist::DIST_FIRST_ELEMENT_B32>(reinterpret_cast<__ubuf__ uint32_t *>(scaleUb),
                                                                (RegTensor<uint32_t> &)scaleReg, pregFullB32);
    }

    template <class TensorSrc, class TensorDst, class TensorScale>
    CATLASS_DEVICE void operator()(TensorSrc const &ubIn, TensorDst const &ubOut, TensorScale const &ubScale,
                                   uint16_t nLoops)
    {
