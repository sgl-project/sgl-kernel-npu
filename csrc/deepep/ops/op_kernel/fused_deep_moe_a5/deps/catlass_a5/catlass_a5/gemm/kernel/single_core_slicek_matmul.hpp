...
public:
    ...
    using LayoutAOut = std::conditional_t<std::is_void_v<PrologueA>, typename BlockMmad::LayoutA,
                                          typename LayoutHelper<PrologueA, true>::type>;
    using LayoutBOut = std::conditional_t<std::is_void_v<PrologueB>, typename BlockMmad::LayoutB,
                                          typename LayoutHelper<PrologueB, true>::type>;
    ...
    // Decide whether PaddingA/PaddingB is enabled.
...
protected:
    static size_t GetPaddingSizeA(const Arguments &args)
    {
        if constexpr (PrologueA::paddingTag == PaddingTag::PADDING_BLOCK_ND) {
            return PaddingA::GetWorkspaceSize(args.problemShape.m(), args.problemShape.k(), L1TileShape::M,
                                              L1TileShape::K);
    ...
    static size_t GetPaddingSizeB(const Arguments &args)
    {
        if constexpr (PrologueB::paddingTag == PaddingTag::PADDING_BLOCK_ND) {
            return PaddingB::GetWorkspaceSize(args.problemShape.k(), args.problemShape.n(), L1TileShape::K,
                                              L1TileShape::N);
