...
#endif
template <class ArchTag_, class ElementSrc_, class ElementScale_, class ElementPerToken_, class ElementDst_,
          class TileShape_>
struct TilePerTokenDequant {
    ...
    static_assert(std::is_same_v<ArchTag, Arch::Ascend950> && std::is_same_v<ElementSrc, int32_t> &&
    ...
    static constexpr CastTrait ctInt322Fp32 = {RegLayout::UNKNOWN, SatMode::UNKNOWN, MaskMergeMode::ZEROING,
                                               AscendC::RoundMode::CAST_RINT};
    static constexpr CastTrait ctFp322Half = {RegLayout::ZERO, SatMode::NO_SAT, MaskMergeMode::ZEROING,
                                              AscendC::RoundMode::CAST_RINT};
    static constexpr CastTrait ctHalf2Fp32Zero = {RegLayout::ZERO, SatMode::UNKNOWN, MaskMergeMode::ZEROING,
                                                  AscendC::RoundMode::UNKNOWN};
    static constexpr CastTrait ctHalf2Fp32One = {RegLayout::ONE, SatMode::UNKNOWN, MaskMergeMode::ZEROING,
                                                 AscendC::RoundMode::UNKNOWN};

    template <class T, LoadDist Dist = LoadDist::DIST_NORM>
    __simd_callee__ inline void LoadInput(RegTensor<float> &dstVreg, __ubuf__ T *srcUb, MaskReg &mask, MaskReg &maskB16)
    {
    ...
    __simd_vf__ void perTokenScaleDequant(__ubuf__ ElementDst *dstUb, __ubuf__ ElementSrc *srcUb,
                                          __ubuf__ ElementScale *scaleUb, __ubuf__ ElementPerToken *perTokenUb,
                                          uint16_t m, uint16_t nLoops, uint32_t tailN)
    {
        ...
        for (uint16_t i = 0; i < m; i++) {
            if constexpr (AscendC::IsSameType<ElementPerToken, float>::value) {
                LoadInput<ElementPerToken, LoadDist::DIST_BRC_B32>(perTokenVreg, perTokenUb + i, pregFull, pregFullB16);
            } else {
                LoadInput<ElementPerToken, LoadDist::DIST_BRC_B16>(perTokenVreg, perTokenUb + i, pregFull, pregFullB16);
            }
            for (uint16_t j = 0; j < nLoops; j++) {
                LoadInput<ElementSrc>(srcVreg, srcUb + i * N_BASE_SIZE + j * FLOAT_REP_SIZE, pregFull, pregFullB16);
                LoadInput<ElementScale>(scaleVreg, scaleUb + j * FLOAT_REP_SIZE, pregFull, pregFullB16);
            ...
            LoadInput<ElementSrc>(srcVreg, srcUb + i * N_BASE_SIZE + nLoops * FLOAT_REP_SIZE, pregTailN, pregTailNB16);
            LoadInput<ElementScale>(scaleVreg, scaleUb + nLoops * FLOAT_REP_SIZE, pregTailN, pregTailNB16);
    ...
    CATLASS_DEVICE void operator()(TensorDst const &ubOut, TensorSrc const &ubIn, TensorScale const &ubScale,
