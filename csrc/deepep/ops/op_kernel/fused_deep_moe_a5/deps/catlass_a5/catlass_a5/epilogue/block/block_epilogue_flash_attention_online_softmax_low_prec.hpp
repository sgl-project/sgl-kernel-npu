...
public:
                                      ...
                                      Arch::CrossCoreFlag softmaxReadyFlag)
    {
        ...
        ComputeExpSubSum16<ElementOutput, ElementInput>(pAddr, sAddr, nowMaxAddr, nowSumAddr, m, nLoops, tailN,
                                                        blockStride, nRound);
                                      ...
                                      uint32_t maskType)
    {
        ...
        ComputeScaleAndMaxMask<ElementInput>(sAddr, nowMaxAddr, maskUbAddr, m, nLoops, tailN, nPadding, scaleValue,
                                             nRound);
...
private:
    ...
    __simd_vf__ inline void ComputeScaleAndMax(__ubuf__ ElementS *srcUb, __ubuf__ ElementS *newMaxUb, uint16_t m,
                                               ...
                                               uint16_t S2BaseSize)
    {
        ...
        for (uint16_t i = 0; i < m; ++i) {
            ...
            for (uint16_t j = 0; j < nLoops; ++j) {
                ...
                StoreAlign<ElementS, StoreDist::DIST_NORM_B16>(srcUb + i * S2BaseSize + j * HALF_REP_SIZE, srcVreg,
                                                               pregFull);
            ...
            StoreAlign<ElementS, StoreDist::DIST_NORM_B16>(srcUb + i * S2BaseSize + nLoops * HALF_REP_SIZE, srcVreg,
                                                           pregTailN);
            Max(maxSrcVreg, maxSrcVreg, srcVreg, pregFull);
                                                   ...
                                                   uint16_t S2BaseSize)
    {
        ...
        for (uint16_t i = 0; i < m; ++i) {
            ...
            for (uint16_t j = 0; j < nLoops; ++j) {
                ...
                StoreAlign<ElementS, StoreDist::DIST_NORM_B16>(srcUb + i * S2BaseSize + j * HALF_REP_SIZE, srcVreg_mask,
                                                               pregFull);
                Max(maxSrcVreg, maxSrcVreg, srcVreg, pregFull);
            }
            LoadAlign(srcVreg, srcUb + i * S2BaseSize + nLoops * HALF_REP_SIZE);
    ...
    template <typename ElementS>
    __simd_vf__ inline void UpdateMax(__ubuf__ ElementS *nowMaxUb, __ubuf__ float *lastMaxUb, uint16_t mLoops,
                                      uint32_t tailM)
    {
        using namespace AscendC::MicroAPI;
        constexpr static CastTrait castTraitZero = {
    ...
    __simd_vf__ inline void ComputeExpSubSum16(__ubuf__ ElementP *expUb, __ubuf__ ElementS *srcUb,
                                               ...
                                               uint16_t nLoops, uint32_t tailN, uint32_t blockStride,
                                               uint16_t S2BaseSize)
    {
        using namespace AscendC::MicroAPI;
        constexpr static CastTrait castTraitZero = {
