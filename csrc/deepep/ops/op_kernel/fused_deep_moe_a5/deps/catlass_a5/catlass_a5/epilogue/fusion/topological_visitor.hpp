...
struct TopologicalVisitor : VisitorImpl<Ops...> {
    ...
    struct Callbacks : CallbacksImpl {
        ...
        CATLASS_DEVICE auto visit_node(TensorC const &tensorTile, MatrixCoord const &alignedTileShape,
                                       MatrixCoord const &globalOffset, Seq /*edges_seq*/,  // 形如 tla::seq<child...>
                                       Args const &...args)
        {
            if (visited_[I]) {
            ...
            auto child_outputs = collect_children<Stage, I, ArchTag, TensorC>(tensorTile, alignedTileShape,
                                                                              globalOffset, Seq{}, args...);
        ...
        CATLASS_DEVICE auto collect_children(TensorC const &tensorTile, MatrixCoord const &alignedTileShape,
                                             MatrixCoord const &globalOffset, tla::seq<ChildIs...> /*seq*/,
                                             Args const &...args)
        {
            return tla::tuple<decltype(this->template visit_node<Stage, ChildIs, ArchTag, TensorC>(
                ...
                this->template visit_node<Stage, ChildIs, ArchTag, TensorC>(tensorTile, alignedTileShape, globalOffset,
                                                                            decltype(tla::get<ChildIs>(EdgeTuple{})){},
                                                                            args...)...);
        }

        // 展开 child_outputs 元组并调用第 I 个算子
        template <VisitStage Stage, int I, class ArchTag, class TensorC, class ChildOutputs, int... Js>
        CATLASS_DEVICE auto call_current_expand(TensorC const &tensorTile, MatrixCoord const &alignedTileShape,
                                                MatrixCoord const &globalOffset, ChildOutputs const &child_outputs,
                                                tla::seq<Js...>)
        {
            return tla::get<I>(this->callbacks_tuple)
                .template visit<Stage, ArchTag>(tensorTile, alignedTileShape, globalOffset,
                                                tla::get<Js>(child_outputs)...);
        }

        template <VisitStage Stage, int I, class ArchTag, class TensorC, class ChildOutputs>
        CATLASS_DEVICE auto call_current(TensorC const &tensorTile, MatrixCoord const &alignedTileShape,
                                         MatrixCoord const &globalOffset, ChildOutputs const &child_outputs)
        {
            constexpr int Num = tla::tuple_size<ChildOutputs>::value;
            return call_current_expand<Stage, I, ArchTag, TensorC>(tensorTile, alignedTileShape, globalOffset,
        ...
        CATLASS_DEVICE auto visit(TensorC const &tensorTile, MatrixCoord const &alignedTileShape,
                                  MatrixCoord const &globalOffset, Args const &...args)
        {
            // 每次visit需要重置缓存标记，防止重复访问一个节点多次
            reset_flags();
            constexpr int R = sizeof...(Ops);
            using RootEdges = decltype(tla::get<R - 1>(EdgeTuple{}));
            return visit_node<Stage, R - 1, ArchTag, TensorC>(tensorTile, alignedTileShape, globalOffset, RootEdges{},
                                                              args...);
        }
    };
