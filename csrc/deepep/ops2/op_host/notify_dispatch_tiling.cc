                                               ...
                                               NotifyDispatchTilingData &tilingData, std::string &commGroup)
{
    ...
    if (profileEnablePtr != nullptr) {
        tilingData.notifyDispatchInfo.profileEnable = static_cast<uint32_t>(*profileEnablePtr);
    }
    if (profileBufferBytesPtr != nullptr) {
        tilingData.notifyDispatchInfo.profileBufferBytes = static_cast<uint64_t>(*profileBufferBytesPtr);
    }
    if (profileLaunchIdPtr != nullptr) {
        tilingData.notifyDispatchInfo.profileLaunchId = static_cast<uint32_t>(*profileLaunchIdPtr);
    }
