...
#include "ge_helper.h"
#include "sparse_flash_attention_ops_compat.h"
#include "sparse_flash_attention_tiling_data.h"

/*
 * Adapted from vllm-ascend csrc/attention/sparse_flash_attention. Differences from upstream, all of
 * them forced by building outside a CANN custom-op package rather than chosen:
 *
 *   - gert::TilingContext          -> sglang::ge_helper::TilingContext (there is no GE graph here;
 *                                     the launcher fakes a tiling context out of the torch tensors).
 *   - register/tilingdata_base.h   -> sparse_flash_attention_tiling_data.h (POD structs).
 *   - err/ops_err.h                -> sparse_flash_attention_ops_compat.h.
 *   - platform/soc_spec.h NpuArch  -> SFANpuArch, derived from PlatformAscendC::GetSocVersion()
 *                                     instead of GetCurNpuArch(); see GetNpuInfo().
 *   - fe::PlatFormInfos            -> dropped; the platform comes from PlatformAscendCManager.
 *   - "tiling/tiling_api.h"        -> added. It is what declares namespace AscendC on the host side,
 *                                     which upstream's tiling.cpp gets transitively through
 *                                     register/tilingdata_base.h; dropping that header left its
 *                                     `using namespace AscendC;` with nothing to name. Including it
 *                                     explicitly is what csrc/sparse_attn_sharedkv does, and keeps
 *                                     upstream's using-directive working rather than deleting it.
 *   - IMPL_OP_OPTILING registration and the aclnn/infershape/def translation units are gone: the
 *     launcher runs SFAInfoParser / SFATilingCheck / SFAMlaTiling itself and allocates the outputs.
 *
 * Two upstream type names are also renamed, for a reason unrelated to CANN: upstream declares
 * KvStorageMode and InnerSplitParams unprefixed in namespace optiling, and csrc/sparse_attn_sharedkv
 * declares its own optiling::KvStorageMode with *different* enumerator values (its PAGE_ATTENTION is
 * 2, upstream's is 1). Both tilings end up in one shared library, so the pair is an ODR violation
 * waiting for the first translation unit that includes both headers. They are SFAKvStorageMode and
 * SFAInnerSplitParams here. Note SFAInnerSplitParams (a tiling-internal struct) is distinct from
 * SparseFlashAttentionInnerSplitParams (a tiling-data struct read by the kernel); upstream has both.
 *
 * Everything else -- every shape, dtype and layout check, the split/UB arithmetic, the tiling-key
 * axes -- is upstream's, kept line-for-line so it stays diffable.
 */
...
/*
 * Replaces the NpuArch enum from CANN's "platform/soc_spec.h", which upstream reads via
 * PlatformAscendC::GetCurNpuArch(). Neither the header nor that accessor is used anywhere else in
 * this repo, so rather than depend on them the arch is derived from GetSocVersion() -- the accessor
 * the rest of the repo already relies on. Only the two values upstream compares against are needed.
 */
enum class SFANpuArch : uint32_t {
    DAV_2201 = 0,  // A2 / A3   (ascend910b, ascend910_93)   -> op_kernel/arch22
    DAV_3510 = 1,  // A5 / 950  (ascend910_95, ascend950)     -> op_kernel/arch35
};
// ------------------算子原型索引常量定义----------------
// Inputs Index
...
enum class SFAKvStorageMode : uint32_t { BATCH_CONTINUOUS = 0, PAGE_ATTENTION = 1 };
...
struct SFAInnerSplitParams {
    uint32_t s1GBaseSize = 1;
    uint32_t s2BaseSize = 1;
};

// -----------算子TilingData定义---------------
// The tiling data structs themselves live in sparse_flash_attention_tiling_data.h; upstream declares
// them here with BEGIN_TILING_DATA_DEF, which needs the CANN op-project macro headers.
...
std::string SFADebugTilingContext(const ge_helper::TilingContext *context);
...
struct SFATilingInfo {
    ...
    // Base Param
    SFANpuArch npuArch = SFANpuArch::DAV_2201;
    bool isA5 = false;
    uint32_t bSize = 0;
    uint32_t n1Size = 0;
    uint32_t n2Size = 0;
    ...
    SFAKvStorageMode kvStorageMode = SFAKvStorageMode::BATCH_CONTINUOUS;
...
public:
    ...
    // Upstream hands these to the GE context (SetTilingData/SetBlockDim/SetWorkspaceSize). With no
    // context to hand them to, the launcher reads them back instead.
    const SparseFlashAttentionTilingDataMla &GetTilingData() const
    {
        return tilingData_;
    }
    ...
    uint64_t GetWorkspaceSizeBytes() const
    {
...
private:
    ...
    // The {} is load-bearing, and is a change from upstream. TilingContext::GetTilingTensor() keys
    // its device-side tiling cache on the struct's complete byte image, padding included, so the
    // padding has to be deterministic or every call misses the cache and claims another device slot.
    // Upstream did not need this: its tiling data was a CANN TilingData class that serialised only
    // the declared fields. Value-initialising here zeroes the whole object, padding included.
...
private:
    ...
    SFAKvStorageMode kvStorageMode_ = SFAKvStorageMode::BATCH_CONTINUOUS;
    ...
    uint32_t aicNum_ = 0;
    uint32_t aivNum_ = 0;
    SFANpuArch npuArch_ = SFANpuArch::DAV_2201;
    bool isA5_ = false;
...
{
public:
    explicit SFAInfoParser(const ge_helper::TilingContext *context) : context_(context) {}
...
public:
    ...
    const ge_helper::TilingContext *context_ = nullptr;
