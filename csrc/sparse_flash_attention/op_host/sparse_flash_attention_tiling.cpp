...
// Upstream also defines SFADebugTilingContext() here, an unused debug dump built on
// GetComputeNodeInfo(); ge_helper::TilingContext has no counterpart, so it is dropped.
...
/*
 * Upstream pushes block dim, tiling key, workspace size and the serialised tiling data into the GE
 * tiling context. There is no GE context here: ge_helper::TilingContext deletes SetBlockDim() and
 * SetTilingKey() precisely because a direct kernel launch has nowhere to put them. So each of these
 * is kept on the tiling object and read back by the launcher (GetBlockDim / GetWorkspaceSizeBytes /
 * GetTilingData), which is what csrc/sparse_attn_sharedkv does.
 *
 * SetScheduleMode(1) (batch mode) has no equivalent either. It is a GE scheduling hint for graph
 * execution and does not affect what the kernel computes.
 */
ge::graphStatus SFAMlaTiling::SetBlockDim(uint32_t blockDim) const
{
    (void)blockDim;  // already in blockDim_; see GetBlockDim()
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus SFAMlaTiling::SetTilingKey(uint64_t tilingKey) const
{
    (void)tilingKey;  // already in tilingData_.dispatchKey; see GenTilingKey()
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus SFAMlaTiling::SetWorkspaceSize(uint64_t workspaceSize) const
{
    (void)workspaceSize;  // already in workspaceSize_; see GetWorkspaceSizeBytes()
    return ge::GRAPH_SUCCESS;
}

/*
 * Upstream's SetTilingData() serialised the tiling data into the GE context's raw tiling buffer with
 * TilingDef::SaveToBuffer(). Both halves of that are gone: the tiling data is a POD struct here (no
 * SaveToBuffer), and ge_helper::TilingContext deletes GetRawTilingData(). The launcher copies the
 * struct into a device tensor with TilingContext::GetTilingTensor(GetTilingData()) instead.
 */
...
/*
 * Upstream builds the key with GET_TPL_TILING_KEY (from ascendc/host_api/tiling/template_argument.h),
 * which encodes the kernel template arguments for the CANN op packer so it can pick the matching
 * pre-compiled binary. There is one binary here, so the same axes are packed into the tiling data as
 * a dispatch key and the kernel entry switches on it at run time.
 *
 * One axis has to be *added*: the element dtype. Upstream gets it for free because the packer compiles
 * a separate binary per dtype and the kernel branches on the ORIG_DTYPE_QUERY macro the packer injects.
 * With a single binary that macro does not exist, so the dtype travels in the key instead.
 */
void SFAMlaTiling::GenTilingKey()
{
    const uint32_t layoutQuery = static_cast<uint32_t>(sfaInfo_->qLayout);
    const uint32_t layoutKV = static_cast<uint32_t>(sfaInfo_->kvLayout);
    const uint32_t pageAttention = (sfaInfo_->kvLayout == SFALayout::PA_BSND) ? 1U : 0U;
    const uint32_t templateMode = (perfMode_ == SFAPerfMode::V_TEMPLATE_MODE) ? SGL_SFA_TEMPLATE_V : SGL_SFA_TEMPLATE_C;
    const uint32_t isSplitG = static_cast<uint32_t>(sfaInfo_->gSize > 64);  // N1 > 128时核间切G
    const uint32_t dtype = (sfaInfo_->inputQType == ge::DT_BF16) ? SGL_SFA_DT_BF16 : SGL_SFA_DT_FP16;

    tilingKey_ = SGL_SFA_MAKE_KEY(dtype, layoutQuery, layoutKV, templateMode, isSplitG, pageAttention);
    tilingData_.dispatchKey = tilingKey_;

    OP_LOGI(sfaInfo_->opName, "SFA dispatchKey: %lu.", tilingKey_);
}
...
void SFAMlaTiling::SplitBalanced()
{
    ...
    SFAInnerSplitParams innerSplitParams;
...
void SFAMlaTiling::FillTilingBaseParamsMla()
{
    ...
    tilingData_.baseParams.preTokens = sfaInfo_->preTokens;
    tilingData_.baseParams.nextTokens = sfaInfo_->nextTokens;
    ...
    tilingData_.baseParams.attentionMode = sfaInfo_->attentionMode;
    tilingData_.baseParams.returnSoftmaxLse = sfaInfo_->returnSoftmaxLse;
    tilingData_.baseParams.isActualLenDimsNull = sfaInfo_->actualQSeqLenFlag ? 0U : 1U;
    tilingData_.baseParams.isActualLenDimsKVNull = sfaInfo_->actualSeqLenFlag ? 0U : 1U;
}
...
ge::graphStatus TilingSparseFlashAttention(ge_helper::TilingContext *context)
{
...
/*
 * Upstream also defines TilingPrepareForSparseFlashAttention(gert::TilingParseContext*) here, a no-op
 * that only ever ran via the .TilingParse<>() leg of the GE registration. With that registration gone
 * nothing can reach it, and gert::TilingParseContext is not declared by the headers this file still
 * includes, so it is dropped along with SparseFlashAttentionCompileInfo's only use.
 */
...
ge::graphStatus SFATilingCheck::CheckSingleParaSparseBlockSize() const
{
    ...
    OP_CHECK_IF((npuArch_ == SFANpuArch::DAV_3510 && *opParamInfo_.sparseBlockSize != 1),
...
{
    if (kvStorageMode_ != SFAKvStorageMode::PAGE_ATTENTION) {
        return ge::GRAPH_SUCCESS;
    }
...
{
    if (kvStorageMode_ == SFAKvStorageMode::BATCH_CONTINUOUS) {
...
ge::graphStatus SFAInfoParser::GetNpuInfo()
{
    ...
    /*
     * Upstream: npuArch_ = ascendcPlatform.GetCurNpuArch(). That accessor and the NpuArch enum come
     * from CANN's "platform/soc_spec.h", which nothing else in this repo uses; the SOC version is the
     * accessor the rest of the repo relies on, and it carries the same distinction.
     *
     * Only arch22 (A2/A3) is wired up -- see the kernel dispatch in
     * op_kernel/sparse_flash_attention.cpp -- so an A5/950 part is rejected here rather than being
     * allowed through to a kernel that was never built for it. The arch35 sources are vendored
     * alongside for whoever adds that target.
     */
    ...
    if (socVersion == platform_ascendc::SocVersion::ASCEND910B ||
        ...
        npuArch_ = SFANpuArch::DAV_2201;
    } else {
        OPS_REPORT_VECTOR_INNER_ERR(
            opName_,
            "sparse_flash_attention is built for Ascend910B/Ascend910_93 (op_kernel/arch22) only; "
            "this SOC is not supported.");
        return GRAPH_FAILED;
    }
    isA5_ = false;
...
ge::graphStatus SFAInfoParser::GetKvStorageMode()
{
    if (kvLayout_ == SFALayout::PA_BSND) {
        kvStorageMode_ = SFAKvStorageMode::PAGE_ATTENTION;
    } else {
        kvStorageMode_ = SFAKvStorageMode::BATCH_CONTINUOUS;
    }
    // kv存储模式基准值
    return ge::GRAPH_SUCCESS;
}
...
void SFAInfoParser::GenerateInfo(SFATilingInfo &sfaInfo)
{
    ...
    sfaInfo.pageAttentionFlag = (kvStorageMode_ == SFAKvStorageMode::PAGE_ATTENTION);
...
/*
 * Upstream registers the tiling with GE here:
 *
 ...
 *
 * There is no GE op registry in this build -- the launcher in sparse_flash_attention.cpp drives the
 * same three stages TilingSparseFlashAttention() does (parse, check, DoOpTiling), separately so that
 * a failure says which stage rejected the call -- and IMPL_OP_OPTILING lives in
 * "register/op_def_registry.h",
 * a CANN op-project header. Registering it would also put an op named SparseFlashAttention into a
 * custom vendor package, which shadows CANN's own build of that name via load_priority in
 * opp/vendors/config.ini. That is exactly what this port must not do.
 */
}  // namespace optiling
