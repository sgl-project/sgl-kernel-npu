...
// cuda-graph replay cache: tiling data is shape/dtype/soc-derived, so one entry per
// distinct (batch,numHeads,kvHeads,embed,blockSize,topK,...) combo. static to avoid
// linkage clash with other host ops in the same .so.
static uint32_t actualCaptureNum = 0;
static std::unordered_map<uint64_t, uint32_t> captureMap;
// Persistent host-side tiling storage: the first-sight H2D copy (aclrtMemcpy) is a
// device op captured into the cuda-graph. On replay it re-runs from the source
// address -- if that were a stack-local &tilingData, the frame would be gone and
// the copy would read garbage, corrupting the tiling. A static array gives each
// capture index a STABLE address holding its own (never-overwritten) tiling, so
// the replayed copy stays correct.
    ...
    int64_t block_size, int64_t top_k, int64_t inner_precise)
{
    ...
    if (captureMap.find(hashValue) != captureMap.end()) {
        ...
        // The copy is captured into the cuda-graph; on replay it re-reads the
        // stable slot (which still holds THIS shape's tiling), not a stale stack.
    ...
    // ---- launch ----
    // ZERO-INIT workspace + softmaxLse: the kernel may read-before-write these
    // scratch buffers (intermediate accumulation). at::empty leaves stale data
    // from the caching allocator's recycled memory -> non-deterministic garbage
    // (correct in isolated replay with a clean allocator, garbage under service
    // memory pressure). at::zeros guarantees a clean 0 baseline each call.
