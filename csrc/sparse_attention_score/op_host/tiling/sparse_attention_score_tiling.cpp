                        ...
                        uint32_t &blockDim)
{
    ...
    // blockDim = min(totalTaskNum, aicNum) -- Cube-core count drives the 1D split.
    ...
    // ---- CalculateWorkSpace ----
    // Full workspace = libapi (system) + identityIdx + per-phase buffers. The kernel
    // recovers the user part via GetUserWorkspace(workspace) (skips the libapi prefix),
    // so the HOST_API allocates exactly workSpaceSize bytes.
