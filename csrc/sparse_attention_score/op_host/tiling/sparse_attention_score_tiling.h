...
// ascend910_93/ascend950 map to SocVersion code 4 in SASATiling; 910B (arch22) is
// any other code. PlatformAscendCManager::GetInstance()->GetSocVersion() returns
// the platform_ascendc::SocVersion enum whose underlying value matches this.
...
// (ge_helper::TilingContext deletes SetBlockDim/SetTilingKey/GetRawTilingData/
// GetPlatformInfo, so SASATiling cannot be reused verbatim).
struct SAInfo {
...
public:
    ...
    // GenerateTilingKey/FillTilingData) without the gert::TilingContext dependency.
    // ParseSeqlens is dropped (dead code: it read actualSeqLengths/Kv host pointers
    // that no later tiling step consumes, and the tiling data struct has no seq_len
    // field -- so tiling is cuda-graph safe, no device->host read needed).
    // workspaceSize is the FULL workspace (libapi + user), matching what the kernel
    // receives via its workspace GM arg; the HOST_API allocates at::empty({workspaceSize}).
