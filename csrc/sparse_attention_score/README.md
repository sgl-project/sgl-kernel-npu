...
when available, with no PTA plugin, no `ASCEND_CUSTOM_OPP_PATH`, and no manual
env export.

## Overview

MiniMax-M3 sparse attention runs in two steps:
1. **Indexer** — scores KV blocks and selects top-k (separate op, `minimax_indexer`).
2. **Main attention** — attention over the selected blocks (**this op**).

This kernel implements step 2 as a native aclnn cube kernel (CUTLASS-style
...
- No on/off gate, no env var (`SGLANG_MINIMAX_NPU_NATIVE_DECODE`/`VERIFY`/`SPARSE_LIB`
  removed). The backend primes the probe at `__init__` (pre cuda-graph capture).
...
- **Single-op unit test**: `test_native_sas.py` (native vs triton bit-exact) +
  `test_native_sas_graph.py` (eager vs cuda-graph replay).
