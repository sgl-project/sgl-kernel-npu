...
/*!
 * \file minimax_indexer_service_cube.h
 * \brief use 5 buffer for matmul l1, better pipeline
 */
#ifndef MINIMAX_INDEXER_SERVICE_CUBE_H
#define MINIMAX_INDEXER_SERVICE_CUBE_H
...
public:
    ...
    __aicore__ inline void ComputeMm1(const MICommon::RunInfo &runInfo);

    static constexpr IsResetLoad3dConfig LOAD3DV2_CONFIG = {true, true};  // isSetFMatrix isSetPadding;
    static constexpr uint64_t KEY_BUF_NUM = 3;
    static constexpr uint64_t QUERY_BUF_NUM = 2;
    // L0 single-buffer: L0B (D×N = 128×256×2B = 64KB) exactly fills L0B at N_tile=256,
    // so no room for double-buffer. With 1 L0 pass per 2-block tile there is no
    // within-tile L0 overlap to lose, and tiles are handshake-serialized anyway.
    // (Stage M config; the L0_BUF_NUM=2 + N_tile=128 variant breaks <=32k output.)
    static constexpr uint64_t L0_BUF_NUM = 1;
    ...
    // N tile = full 2-block tile (256): 1 L0 pass per Cube<->Vector tile instead of 2,
    // halving per-tile Mmad + fixpipe count. Requires L0B single-buffer (see L0_BUF_NUM).
    static constexpr uint64_t S2_BASIC_BLOCK_L0 = 256;
...
protected:
    __aicore__ inline void Fixp(uint64_t s1gGmOffset, uint64_t s2GmOffset, uint64_t s1gL0RealSize,
                                uint64_t s2L0RealSize, const MICommon::RunInfo &runInfo);
    __aicore__ inline void ComuteL0c(uint64_t s1gL0RealSize, uint64_t s2L0RealSize, const MICommon::RunInfo &runInfo);
    __aicore__ inline void LoadKeyToL0b(uint64_t s2L0Offset, uint64_t s2L1RealSize, uint64_t s2L0RealSize,
                                        const MICommon::RunInfo &runInfo);
    __aicore__ inline void LoadQueryToL0a(uint64_t s1gL1Offset, uint64_t s1gL0Offset, uint64_t s1gL1RealSize,
                                          uint64_t s1gL0RealSize, const MICommon::RunInfo &runInfo);
    __aicore__ inline void QueryNd2Nz(uint64_t s1gL1RealSize, uint64_t s1gL1Offset, const MICommon::RunInfo &runInfo);
    __aicore__ inline void KeyNd2Nz(uint64_t s2L1RealSize, uint64_t s2GmOffset, const MICommon::RunInfo &runInfo);
    __aicore__ inline void KeyNd2NzForPA(uint64_t s2L1RealSize, uint64_t s2GmOffset, const MICommon::RunInfo &runInfo);
    GlobalTensor<int32_t> blkTableGm_;
    GlobalTensor<int32_t> reqToTokenGm_;
    GlobalTensor<int32_t> reqPoolIdxGm_;
...
template <typename MIT>
__aicore__ inline void MIMatmul<MIT>::InitBuffers(TPipe *pipe)
{
...
__aicore__ inline void MIMatmul<MIT>::ComputeMm1(const MICommon::RunInfo &runInfo)
{
    ...
    for (uint64_t s2GmOffset = 0; s2GmOffset < s2ProcessSize; s2GmOffset += S2_BASIC_BLOCK) {
        ...
        // s1gProcessSize never exceeds 2x s1g basic block
...
template <typename MIT>
__aicore__ inline void MIMatmul<MIT>::KeyNd2Nz(uint64_t s2L1RealSize, uint64_t s2GmOffset,
                                               const MICommon::RunInfo &runInfo)
{
    ...
    while (s2L1Offset < s2L1RealSize) {
        ...
        // laid out as S2_BASIC_BLOCK_L0*D_BASIC_BLOCK_L0 in L1 for MTE1
        // determine which L0 tile based on s2 offset (supports up to 2 tiles)
...
// blkNum, blkSize, N2, D
template <typename MIT>
__aicore__ inline void MIMatmul<MIT>::KeyNd2NzForPA(uint64_t s2L1RealSize, uint64_t s2GmOffset,
                                                    const MICommon::RunInfo &runInfo)
{
    uint64_t s2L1Offset = 0;
    while (s2L1Offset < s2L1RealSize) {
        ...
        // Fused direct page lookup: physical block id is gathered from
        // req_to_token[req_pool_indices[bIdx]][blk * block_size] // block_size,
        // so the host-side arange+gather+div block_table construction is skipped.
        uint64_t physBlk;
        if (constInfo_.directMode) {
            physBlk = reqToTokenGm_.GetValue(reqPoolIdxGm_.GetValue(runInfo.bIdx) * constInfo_.maxTokenSlots +
                                             s2BlkId * constInfo_.kCacheBlockSize) /
                      constInfo_.kCacheBlockSize;
        } else {
            physBlk = blkTableGm_.GetValue(runInfo.bIdx * constInfo_.maxBlockNumPerBatch + s2BlkId);
        }
        uint64_t keyGmOffset = physBlk * constInfo_.kCacheBlockSize * constInfo_.kHeadNum * constInfo_.headDim +
...
// batch, s1, n2, g, d
template <typename MIT>
__aicore__ inline void MIMatmul<MIT>::QueryNd2Nz(uint64_t s1gL1RealSize, uint64_t s1gGmOffset,
                                                 const MICommon::RunInfo &runInfo)
{
    Nd2NzParams nd2nzPara;
    nd2nzPara.ndNum = 1;
    nd2nzPara.nValue = s1gL1RealSize;  // row count
    nd2nzPara.dValue = constInfo_.headDim;
    nd2nzPara.srcDValue = constInfo_.headDim;
    nd2nzPara.dstNzC0Stride = CeilAlign(s1gL1RealSize, (uint64_t)BLOCK_CUBE);  // aligned to 16-element blocks
    nd2nzPara.dstNzNStride = 1;
    nd2nzPara.srcNdMatrixStride = 0;
    nd2nzPara.dstNzMatrixStride = 0;
    // default: one buffer holds at most 2 copies
...
template <typename MIT>
__aicore__ inline void MIMatmul<MIT>::LoadQueryToL0a(uint64_t s1gGmOffset, uint64_t s1gL1Offset, uint64_t s1gL1RealSize,
                                                     uint64_t s1gL0RealSize, const MICommon::RunInfo &runInfo)
{
    ...
    loadData3DParams.padList[3] = 255;  // tail padding does not affect sliding-window results

    // SetLoadToA0Params
    loadData3DParams.mExtension = CeilAlign(s1gL0RealSize, (uint64_t)BLOCK_CUBE);  // M dimension (height) destination
    loadData3DParams.kExtension = constInfo_.headDim;                              // K dimension (width) destination
...
template <typename MIT>
__aicore__ inline void MIMatmul<MIT>::LoadKeyToL0b(uint64_t s2L1Offset, uint64_t s2L1RealSize, uint64_t s2L0RealSize,
                                                   const MICommon::RunInfo &runInfo)
{
...
template <typename MIT>
__aicore__ inline void MIMatmul<MIT>::ComuteL0c(uint64_t s1gL0RealSize, uint64_t s2L0RealSize,
                                                const MICommon::RunInfo &runInfo)
{
...
template <typename MIT>
__aicore__ inline void MIMatmul<MIT>::Fixp(uint64_t s1gGmOffset, uint64_t s2GmOffset, uint64_t s1gL0RealSize,
                                           uint64_t s2L0RealSize, const MICommon::RunInfo &runInfo)
{
...
template <typename MIT>
__aicore__ inline void MIMatmul<MIT>::AllocEventID()
{
...
template <typename MIT>
__aicore__ inline void MIMatmul<MIT>::FreeEventID()
{
