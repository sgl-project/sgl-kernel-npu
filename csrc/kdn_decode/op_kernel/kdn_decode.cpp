// Vector-only recurrent KDA/KDN decode kernel (PTO-ISA).
//
// Upstream source of truth: megagdn-pto/kernels/pto/kdn_decode.cpp.  This copy
// differs only in its launch boundary: GM_ADDR arguments for the ACLRT codegen,
// no ffts plumbing (the ACL launch path sets the FFTS base itself and this
// kernel is vector-only, so it never issues a cross-core sync), and no
// out-of-place `state_out` (sglang always updates the gathered pool in place).
// Keep the recurrence body in sync with upstream.
//
...
#define KDN_BV 32
#endif
#ifndef KDN_KERNEL_NAME
#define KDN_KERNEL_NAME launch_kdn_decode
#endif
#ifndef AICORE
...
KDN_KERNEL_NAME(GM_ADDR q_ptr, GM_ADDR k_ptr, GM_ADDR v_ptr, GM_ADDR g_ptr, GM_ADDR beta_ptr,
                GM_ADDR state_ptr, GM_ADDR out_ptr, GM_ADDR state_indices_ptr,
                GM_ADDR cu_seqlens_ptr, int64_t num_sequences, int64_t seq_len,
                int32_t num_heads, int32_t num_state_slots, float scale, int32_t l2norm)
// clang-format on
{
    kdn_decode_kernel<GDN_D, KDN_V, KDN_BV>(
        ...
        reinterpret_cast<__gm__ half *>(out_ptr),
        reinterpret_cast<__gm__ int32_t *>(state_indices_ptr),
        reinterpret_cast<__gm__ int32_t *>(cu_seqlens_ptr), num_sequences, seq_len, num_heads,
        num_state_slots, scale, l2norm);
}
