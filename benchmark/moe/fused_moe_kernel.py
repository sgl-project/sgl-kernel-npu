...
def fused_moe_kernel(
    ...
    pid = tl.program_id(axis=0)# 0-7
    num_pid_m = tl.cdiv(EM, BLOCK_SIZE_M) # 64 / 16 = 4
    num_pid_n = tl.cdiv(N, BLOCK_SIZE_N) #  64  /32 = 2
    num_pid_in_group = GROUP_SIZE_M * num_pid_n # 4 * 2 
    ...
    if bias_ptr is not None:#Size([4, 64])
        bias = tl.load(
