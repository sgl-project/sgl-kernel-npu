...
GEMMA_PROVIDERS = {"Ascend910": "native", "Ascend950": "aclnn"}


def write_build_target_config(build_lib: Path, target: str) -> None:
    try:
        provider = GEMMA_PROVIDERS[target]
    except KeyError as error:
        raise ValueError(f"Unsupported wheel target: {target!r}") from error

    config_path = Path(build_lib) / "sgl_kernel_npu" / "_build_target.py"
    config_path.parent.mkdir(parents=True, exist_ok=True)
    config_path.write_text(
        "# Generated by the wheel build. Do not edit.\n"
        f'SOC_VERSION = "{target}"\n'
        f'GEMMA_RMS_NORM_PROVIDER = "{provider}"\n',
        encoding="utf-8",
    )
...
class TargetBuildPy(build_py):
    ...
    def run(self):
        ...
        write_build_target_config(Path(self.build_lib), target)
