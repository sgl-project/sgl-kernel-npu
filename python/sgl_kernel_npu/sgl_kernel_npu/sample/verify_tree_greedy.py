...
def verify_tree_greedy_kernel(
    ...
    cur_candidates = tl.load(candidates + cur_tokens_offset, mask=tokens_offset < num_draft_tokens, other=0)
    cur_retrive_index = tl.load(retrive_index + cur_tokens_offset, mask=tokens_offset < num_draft_tokens, other=0)
    cur_target = tl.load(target_predict + cur_tokens_offset, mask=tokens_offset < num_draft_tokens, other=0)
    ...
    for i in range(1, num_draft_tokens):
        ...
        if not rejected:
            ...
            if draft_token == target_token:
                ...
                num_accepted += 1
                tl.store(accept_index + req_idx * num_draft_tokens + num_accepted, draft_idx)
                last_accepted_idx = draft_idx
                found = True
                    
        if not found:
            rejected = True
    target_token = tl.get_element(cur_target, (last_accepted_idx - num_draft_tokens * req_idx,))
