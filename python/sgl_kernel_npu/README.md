...
Released packages are built for the `910` target only, so Ascend 950 users
must build the `950` wheel from source — there is no prebuilt Ascend 950 artifact.
...
ported to the Ascend 950 pipeline model. The wheel build still stages only the ACLNN
