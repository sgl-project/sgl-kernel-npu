...
(Optional) Disable quantization in Decode phase:
```bash
# Disable quantization in low_latency_dispatch; unset or set to 0 to keep quantization enabled
export SGLANG_DEEPEP_BF16_DISPATCH=1
```
> [!WARNING]
> This variable is configured by SGLang framework and only affects the Decode phase.
...
```
> [!WARNING]
> 该变量由 SGLang 框架配置，仅在 Decode 阶段生效。
