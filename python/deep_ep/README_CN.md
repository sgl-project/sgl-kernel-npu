...
|------|-------------|---------|-------------|------|------|
| BF16（不量化） | `None`（默认） | `bfloat16` | — | — | 全部 |
| INT8 动态量化 | `None` + 环境变量 | `int8` | `float32` | 逐 token | 全部 |
| MXFP8 逐 block | `None` + fp8 tuple | `float8_e4m3fn` | `float8_e8m0fnu` | 每 32 元素 | A5/C310 |
| **标量 FP8** | `"scalar_fp8"` | `float8_e4m3fn` | `float32` | 逐 token | A5/C310 |
...
buffer.dispatch(x=(data, fp8_signal_tensor), ...)  # quant_mode=None 默认走 MXFP8
```
