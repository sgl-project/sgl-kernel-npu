...
class Buffer:
    ...
    @log_parameters(["topk_idx"])
    def shmem_low_latency_dispatch(
        self,
        x: torch.Tensor,
        topk_idx: torch.Tensor,
        num_max_dispatch_tokens_per_rank: int,
        ...
        Returns:
            ...
            recv_count: a tensor shaped `[num_local_experts]` with type `torch.int`, indicating how many tokens each
                expert receives. As mentioned before, not all tokens are valid in `recv_x`.
            handle: the communication handle to be used in the `shmem_low_latency_combine` function.
            event: the event after executing the kernel (valid only if `async_finish` is set).
            hook: the receiving hook function (valid only if `return_recv_hook` is set).
        """
        topk_ids = topk_idx.int()
        (
            ...
            event,
            hook,
        ) = self.runtime.shmem_low_latency_dispatch(
            x,
            topk_ids,
            cumulative_local_expert_recv_stats,
            num_max_dispatch_tokens_per_rank,
    ...
    def shmem_low_latency_combine(
        ...
        combined_x, event, hook = self.runtime.shmem_low_latency_combine(
            x,
            topk_ids,
            topk_weights,
            src_info,
