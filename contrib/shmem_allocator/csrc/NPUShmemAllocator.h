...
//    NPU_CHECK_ERROR_MOCK(aclrtGetDeviceCount(&device_count));
...
struct C10_NPU_API MemPool {
    MemPool(
        NPUShmemAllocator::NPUAllocator* allocator = nullptr,
        bool is_user_created = true);

    MempoolId_t id();
    NPUShmemAllocator::NPUAllocator* allocator();

private:
    static std::atomic<CaptureId_t> uid_;
    static std::atomic<CaptureId_t> uuid_;
    NPUShmemAllocator::NPUAllocator* allocator_;
