...
aclError aclrtMallocAlign32(void **ptr, size_t size, aclrtMemMallocPolicy policy) {
    if (thread_local_config.is_interesting_region()) {
        return TorchMemorySaver::instance().malloc(ptr, CANNUtils::cann_ctx_get_device(), size, thread_local_config.current_tag_,
...
void *tms_torch_malloc(ssize_t size, int device, aclrtStream stream) {
    ...
    std::cout << "[torch_memory_saver.cpp] tms_torch_malloc "
              << " size=" << size << " device=" << device << " stream=" << stream << std::endl;
#endif
    SIMPLE_CHECK(thread_local_config.is_interesting_region(), "only support interesting region");
    void *ptr;
    TorchMemorySaver::instance().malloc(&ptr, CANNUtils::cann_device_get(device), size, thread_local_config.current_tag_,
                                        thread_local_config.enable_cpu_backup());
    return ptr;
}
...
void tms_torch_free(void *ptr, ssize_t ssize, int device, aclrtStream stream) {
    ...
    std::cout << "[torch_memory_saver.cpp] tms_torch_free "
              << " ptr=" << ptr << " ssize=" << ssize << " device=" << device << " stream=" << stream << std::endl;
#endif
    SIMPLE_CHECK(thread_local_config.is_interesting_region(), "only support interesting region");
