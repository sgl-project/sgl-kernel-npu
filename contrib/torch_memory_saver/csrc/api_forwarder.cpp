...
aclError call_real_aclrt_malloc_align32(void **ptr, size_t size, aclrtMemMallocPolicy policy) {
    ...
    std::call_once(malloc_flag, []() { real_aclrt_malloc_align32_ = (AclrtMallocFuncAlign32)check_dlsym(dlsym(RTLD_NEXT, "aclrtMallocAlign32")); });

    aclError ret = real_aclrt_malloc_align32_(ptr, size, policy);

#ifdef TMS_DEBUG_LOG
    std::cout << "[torch_memory_saver.cpp] cudaMalloc [MODE NORMAL]"
              << " ptr=" << ptr << " *ptr=" << *ptr << " size=" << size << " ret=" << ret << std::endl;
#endif
...
aclError call_real_aclrt_free(void *ptr) {
    ...
    std::call_once(free_flag, []() { real_aclrt_free_ = (AclrtFreeFunc)check_dlsym(dlsym(RTLD_NEXT, "aclrtFree")); });
