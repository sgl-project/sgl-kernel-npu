...
#define SIMPLE_CHECK(COND, MSG)                                                                                                                   \
    do {                                                                                                                                          \
        if (!(COND)) {                                                                                                                            \
            std::cerr << "[torch_memory_saver.cpp] " << MSG << " file=" << __FILE__ << " func=" << __func__ << " line=" << __LINE__ << std::endl; \
            exit(1);                                                                                                                              \
        }                                                                                                                                         \
    } while (false)
...
inline bool get_bool_env_var(const char *name) {
    ...
    std::cerr << "[torch_memory_saver.cpp] Unsupported environment variable value "
