...
aclError TorchMemorySaver::malloc(void **ptr, int device, size_t size, const std::string &tag, const bool enable_cpu_backup) {
    ...
    std::cout << "[torch_memory_saver.cpp] TorchMemorySaver.cuda_malloc "
              << " ptr=" << ptr << " *ptr=" << *ptr << " size=" << size << " allocHandle=" << allocHandle << " tag=" << tag << std::endl;
#endif
    return ACL_SUCCESS;
}
...
aclError TorchMemorySaver::free(void *ptr) {
    ...
    std::cout << "[torch_memory_saver.cpp] TorchMemorySaver.cuda_free "
              << " ptr=" << ptr << " metadata.size=" << metadata.size << " metadata.allocHandle=" << metadata.allocHandle << " tag=" << metadata.tag
              << std::endl;
#endif
    return ACL_SUCCESS;
}
...
void TorchMemorySaver::pause(const std::string &tag) {
    ...
    for (auto it = allocation_metadata_.begin(); it != allocation_metadata_.end(); ++it) {
        ...
        std::cout << "[torch_memory_saver.cpp] TorchMemorySaver.pause"
                  << " ptr=" << ptr << " metadata.size=" << metadata.size << " metadata.allocHandle=" << metadata.allocHandle << " tag=" << metadata.tag
                  << " filter_tag=" << tag << " metadata.enable_cpu_backup=" << metadata.enable_cpu_backup << std::endl;
    ...
    for (auto it = allocation_metadata_.begin(); it != allocation_metadata_.end(); ++it) {
      ...
      // reduce re-alloc time)
        ...
        std::cout << "[torch_memory_saver.cpp] TorchMemorySaver.resume"
                  << " ptr=" << ptr << " metadata.size=" << metadata.size << " (old)metadata.allocHandle=" << metadata.allocHandle
                  << " (new)newAllocHandle=" << newAllocHandle << " tag=" << metadata.tag << " filter_tag=" << tag
                  << " metadata.enable_cpu_backup=" << metadata.enable_cpu_backup << std::endl;
#endif
