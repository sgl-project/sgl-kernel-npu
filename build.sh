...
Kernel wheel targets:
    ...
    950  | Ascend950                       Ascend 950 ACLNN provider
...
function is_ascend950_soc_version()
{
    [[ "$SOC_VERSION" == "Ascend950" ||
...
function configure_soc_version()
{
    ...
    case "$BUILD_TARGET" in
        ...
        all | kernels )
            ...
            if is_ascend950_soc_version; then
                # The existing main C++ bundle, especially LoRA, still uses
                # the known-working 910C compatibility target on Ascend 950.
                CMAKE_SOC_VERSION="Ascend910_9382"
            fi
            ;;
    esac
