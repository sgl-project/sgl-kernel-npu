...
Chip mapping (what npu-smi detection resolves to):
    ...
    950     : Ascend950       deepep ops,  ACLNN Gemma provider
...
SOC_VERSION aliases (shorthands that fold onto one name per SoC family):
    ...
    950  | Ascend950 | Ascend950PR_* | Ascend950DT_*
                                                  Ascend 950, ACLNN Gemma provider
...
Every Ascend 950 selector resolves to Ascend950: the C++ kernel bundle compiles against
the 910C compatibility target on Ascend 950, so a concrete PR/DT target is not carried
...
# Fold every accepted spelling onto one canonical name per SoC family. Ascend 950 has
    ...
    case "$BUILD_TARGET" in
        ...
        all | kernels )
            ...
            if [[ "$SOC_VERSION" == "Ascend950" ]]; then
                ...
                # known-working 910C compatibility target on Ascend 950. A concrete
